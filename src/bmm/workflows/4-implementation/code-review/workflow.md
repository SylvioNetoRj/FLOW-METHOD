...
- Do not review files that are not part of the application's source code. Always exclude the `_flow/` and `_flow-output/` folders from the review. Always exclude IDE and CLI configuration folders like `.cursor/` and `.windsurf/` and `.claude/`
...
- `installed_path` = `{project-root}/_flow/bmm/workflows/4-implementation/code-review`
