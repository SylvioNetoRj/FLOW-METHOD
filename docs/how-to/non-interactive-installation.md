...
description: Install FLOW using command-line flags for CI/CD pipelines and automated deployments
sidebar:
  order: 2
---

Use command-line flags to install FLOW non-interactively. This is useful for:
...
| `--output-folder <path>` | Output folder path | _flow-output |
...
- `bmm` — FLOW Method Master
- `bmb` — FLOW Builder

Check the [FLOW registry](https://github.com/flow-code-org) for available external modules.
...
- A fully configured `_flow/` directory in your project
- Compiled agents and workflows for your selected modules and tools
- A `_flow-output/` folder for generated artifacts

## Validation and Error Handling

FLOW validates all provided flags:
...
:::note[Still stuck?]
Run with `--debug` for detailed output, try interactive mode to isolate the issue, or report at <https://github.com/SylvioNetoRj/FLOW-METHOD/issues>.
:::
