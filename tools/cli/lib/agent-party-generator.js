...
const AgentPartyGenerator = {
  ...
  generateAgentParty(agentDetails, options = {}) {
    ...
    // Build XML content
    let xmlContent = `<!-- Powered by FLOW-CORE™ -->
<!-- Agent Manifest - Generated during FLOW ${forWeb ? 'bundling' : 'installation'} -->
<!-- This file contains a summary of all ${forWeb ? 'bundled' : 'installed'} agents for quick reference -->
<manifest id="flow/_config/agent-manifest.csv" version="1.0" generated="${new Date().toISOString()}">
  <description>
    Complete roster of ${forWeb ? 'bundled' : 'installed'} FLOW agents with summarized personas for efficient multi-agent orchestration.
...
`;
  ...
  extractAgentDetails(content, moduleName, agentName) {
    try {
      ...
      return {
        id: `flow/${moduleName}/agents/${agentName}.md`,
