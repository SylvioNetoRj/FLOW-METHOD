...
/**
 * Find FLOW directory in project
 */
function findFlowDir(projectDir = process.cwd()) {
  const possibleNames = ['_flow'];

  for (const name of possibleNames) {
    const flowDir = path.join(projectDir, name);
...
async function migrate(directory) {
  ...
  const flowDir = findFlowDir(projectRoot);

  if (!flowDir) {
    console.error(chalk.red('✗ No FLOW installation found in directory'));
    process.exit(1);
  }

  console.log(chalk.blue.bold('🔄 FLOW Custom Module Path Migration'));
  console.log(chalk.dim(`Project: ${projectRoot}`));
  console.log(chalk.dim(`FLOW Directory: ${flowDir}`));
