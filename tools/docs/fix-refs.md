...
description: Corrects workflow, agent, and command references in FLOW documentation
---
...
Fix incorrect references to workflows, agents, and commands in FLOW documentation files.
...
**How-To guides require judgment**: Don't assume experienced. Ask: "Does this task require prior FLOW knowledge?" Early-journey tasks (first PRD, first sprint) are newbie docs. Customization and advanced features are experienced.

**If unclear**: Ask the user "Who is the target audience for this document — new users learning FLOW, or experienced users who know the system?"
...
| `flow_bmm_*` | `flow_bmm_workflow-init` | Internal slash command name, platform-specific |
...
- First mention: Run the `help` workflow (`/flow-help` on most platforms)
...
|----------|----------|-------|
| `workflow-init` | `flow-help` | DEPRECATED - help system replaces initialization |
| `workflow-status` | `flow-help` | DEPRECATED - help system replaces status checking |

### The Help System

The `flow-help` workflow is the modern replacement for both `workflow-init` and `workflow-status`:
...
Users can run `flow-help` to get guidance on what to do next. It detects:
...
3. **Simple names**: Just the workflow name, no `flow_bmm_` prefix, no `/` prefix
