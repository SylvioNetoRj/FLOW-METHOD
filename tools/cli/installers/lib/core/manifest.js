...
class Manifest {
   ...
   */
  async create(flowDir, data, installedFiles = []) {
    ...
    // Get the FLOW version from package.json
    const flowVersion = data.version || require(path.join(process.cwd(), 'package.json')).version;

    // Convert module list to new detailed format
    const moduleDetails = [];
    if (data.modules && Array.isArray(data.modules)) {
      for (const moduleName of data.modules) {
        // Core and BMM modules use the FLOW version
        const moduleVersion = moduleName === 'core' || moduleName === 'bmm' ? flowVersion : null;
   ...
   */
  async read(flowDir) {
    const yamlPath = path.join(flowDir, '_config', 'manifest.yaml');
    const yaml = require('yaml');
   ...
   */
  async update(flowDir, updates, installedFiles = null) {
    const yaml = require('yaml');
    const manifest = (await this._readRaw(flowDir)) || {
      installation: {},
      modules: [],
      ides: [],
    };
   ...
   */
  async addModule(flowDir, moduleName, options = {}) {
    const manifest = await this._readRaw(flowDir);
    if (!manifest) {
    ...
    await this._writeRaw(flowDir, manifest);
  }
   ...
   */
  async removeModule(flowDir, moduleName) {
    const manifest = await this._readRaw(flowDir);
   ...
   */
  async updateModuleVersion(flowDir, moduleName, versionInfo) {
    const manifest = await this._readRaw(flowDir);
    if (!manifest || !manifest.modules) {
      return;
    }
   ...
   */
  async getModuleVersion(flowDir, moduleName) {
    const manifest = await this._readRaw(flowDir);
    if (!manifest || !manifest.modules) {
      return null;
    }
   ...
   */
  async getAllModuleVersions(flowDir) {
    const manifest = await this._readRaw(flowDir);
    if (!manifest || !manifest.modules) {
      return [];
    }
   ...
   */
  async _writeRaw(flowDir, manifestData) {
    const yaml = require('yaml');
    const manifestPath = path.join(flowDir, '_config', 'manifest.yaml');
   ...
   */
  async addIde(flowDir, ideName) {
    const manifest = await this.read(flowDir);
    if (!manifest) {
      throw new Error('No manifest found');
    }
    ...
    if (!manifest.ides.includes(ideName)) {
      ...
      await this.update(flowDir, { ides: manifest.ides });
    }
  }
   ...
   * @param {string} flowDir - Path to flow directory for relative paths
   * @returns {Array} Array of file metadata objects
   */
  async parseInstalledFiles(installedFiles, flowDir) {
    const fileMetadata = [];

    for (const filePath of installedFiles) {
      const fileExt = path.extname(filePath).toLowerCase();
      // Make path relative to parent of flow directory, starting with 'flow/'
      const relativePath = 'flow' + filePath.replace(flowDir, '').replaceAll('\\', '/');
   ...
   * @param {string} relativePath - Relative path starting with 'flow/'
   ...
   */
  async addCustomModule(flowDir, customModule) {
    const manifest = await this.read(flowDir);
    if (!manifest) {
      throw new Error('No manifest found');
    }
    ...
    await this.update(flowDir, { customModules: manifest.customModules });
  }
   ...
   */
  async removeCustomModule(flowDir, moduleId) {
    const manifest = await this.read(flowDir);
   ...
   */
  async getModuleVersionInfo(moduleName, flowDir, moduleSourcePath = null) {
    const os = require('node:os');
    const yaml = require('yaml');

    // Built-in modules use FLOW version (only core and bmm are in FLOW-METHOD repo)
    if (['core', 'bmm'].includes(moduleName)) {
      const flowVersion = require(path.join(getProjectRoot(), 'package.json')).version;
      return {
        version: flowVersion,
        source: 'built-in',
        npmPackage: null,
    ...
    if (moduleInfo) {
      ...
      if (!version) {
        const cacheDir = path.join(os.homedir(), '.flow', 'cache', 'external-modules', moduleName);
    ...
    const cacheDir = path.join(flowDir, '_config', 'custom', moduleName);
   ...
   */
  async checkForUpdates(flowDir) {
    const modules = await this.getAllModuleVersions(flowDir);
    const updates = [];
