...
- `flow-create-architecture` — respects technical preferences during solutioning
- `flow-create-story` — informs story creation with project patterns
- `flow-dev-story` — guides implementation decisions
- `flow-code-review` — validates against project standards
- `flow-quick-dev` — applies patterns when implementing tech-specs
- `flow-sprint-planning`, `flow-retrospective`, `flow-correct-course` — provides project-wide context
...
|----------|----------------|---------|
| **New project, before architecture** | Manually, before `flow-create-architecture` | Document your technical preferences so the architect respects them |
| **New project, after architecture** | Via `flow-generate-project-context` or manually | Capture architecture decisions for implementation agents |
| **Existing project** | Via `flow-generate-project-context` | Discover existing patterns so agents follow established conventions |
| **Quick Flow project** | Before or during `flow-quick-dev` | Ensure quick implementation respects your patterns |
...
Create the file at `_flow-output/project-context.md` and add your rules:

```bash
# In your project root
mkdir -p _flow-output
touch _flow-output/project-context.md
```
...
Run the `flow-generate-project-context` workflow after completing your architecture:

```bash
flow-generate-project-context
```
...
For existing projects, run `flow-generate-project-context` to discover existing patterns:
...
- Patterns flowlve during implementation
- You identify gaps from agent behavior

You can edit it manually at any time, or re-run `flow-generate-project-context` to update it after significant changes.

:::note[File Location]
The default location is `_flow-output/project-context.md`. Workflows search for it there, and also check `**/project-context.md` anywhere in your project.
:::
