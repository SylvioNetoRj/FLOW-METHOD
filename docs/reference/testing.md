...
FLOW provides two testing paths: a built-in QA agent for fast test generation and an installable Test Architect module for enterprise-grade test strategy.
...
| **Setup** | Nothing to install -- included in BMM | Install separately via `npx @flownetbr/flow-method install` |
...
**Trigger:** `QA` or `flow-qa-generate-e2e-tests`
...
- **Documentation:** [TEA Module Docs](https://flow-code-org.github.io/flow-method-test-architecture-enterprise/)
- **Install:** `npx @flownetbr/flow-method install` and select the TEA module
...
Quinn's Automate workflow appears in Phase 4 (Implementation) of the FLOW Method workflow map. A typical sequence:
