...
/**
 * Find FLOW config file in project
 * @param {string} startPath - Starting directory to search from
 * @returns {Object|null} Config data or null
 */
function findFlowConfig(startPath = process.cwd()) {
  // Look for common FLOW folder names
  const possibleNames = ['_flow'];
  ...
  for (const name of possibleNames) {
    ...
    if (fs.existsSync(configPath)) {
      ...
      return {
        ...config,
        flowFolder: path.join(startPath, name),
...
/**
 * Resolve path variables like {project-root} and {flow-folder}
 * @param {string} pathStr - Path with variables
 * @param {Object} context - Contains projectRoot, flowFolder
 * @returns {string} Resolved path
 */
function resolvePath(pathStr, context) {
  return pathStr.replaceAll('{project-root}', context.projectRoot).replaceAll('{flow-folder}', context.flowFolder);
}
...
/**
 * Detect if a path is within a FLOW project
 * @param {string} targetPath - Path to check
 * @returns {Object|null} Project info with flowFolder and cfgFolder
 */
function detectFlowProject(targetPath) {
  let checkPath = path.resolve(targetPath);
  const root = path.parse(checkPath).root;

  // Walk up directory tree looking for FLOW installation
  while (checkPath !== root) {
    const possibleNames = ['_flow'];
    for (const name of possibleNames) {
      const flowFolder = path.join(checkPath, name);
      const cfgFolder = path.join(flowFolder, '_config');
...
async function createIdeSlashCommands(projectRoot, agentName, agentPath, metadata) {
  ...
  const manifestPath = path.join(projectRoot, '_flow', '_config', 'manifest.yaml');
