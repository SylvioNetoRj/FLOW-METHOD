 ...
 * Uses the existing FLOW builder infrastructure for proper formatting
 */
...
function buildFrontmatter(metadata, agentName) {
  ...
  const description = metadata.title || 'FLOW Agent';
...
function buildMenuXml(menuItems) {
  ...
  xml += `    <item cmd="PM or fuzzy match on party-mode" exec="{project-root}/_flow/core/workflows/party-mode/workflow.md">[PM] Start Party Mode</item>\n`;
