 ...
 *
 * This class provides a standardized way to install FLOW artifacts to IDEs
...
class ConfigDrivenIdeSetup extends BaseIdeSetup {
   ...
   * For skill_format platforms, checks for flow-prefixed entries in target_dir
    ...
    if (this.installerConfig?.skill_format && this.configDir) {
      ...
      if (await fs.pathExists(dir)) {
        try {
          ...
          return entries.some((e) => typeof e === 'string' && e.startsWith('flow'));
        } catch {
          return false;
        }
      }
   ...
   */
  async setup(projectDir, flowDir, options = {}) {
    // Check for FLOW files in ancestor directories that would cause duplicates
    if (this.installerConfig?.ancestor_conflict_check) {
      const conflict = await this.findAncestorConflict(projectDir);
      if (conflict) {
        await prompts.log.error(
          `Found existing FLOW skills in ancestor installation: ${conflict}\n` +
            `  ${this.name} inherits skills from parent directories, so this would cause duplicates.\n` +
            `  Please remove the FLOW files from that directory first:\n` +
            `    rm -rf "${conflict}"/flow*`,
        );
        return {
          success: false,
    ...
    // Clean up any old FLOW installation first
    ...
    if (this.installerConfig.targets) {
      return this.installToMultipleTargets(projectDir, flowDir, this.installerConfig.targets, options);
    }

    // Handle single-target installations
    if (this.installerConfig.target_dir) {
      return this.installToTarget(projectDir, flowDir, this.installerConfig, options);
    }
   ...
   */
  async installToTarget(projectDir, flowDir, config, options) {
    ...
    if (!skipStandardArtifacts) {
      ...
      if (!artifact_types || artifact_types.includes('agents')) {
        const agentGen = new AgentCommandGenerator(this.flowFolderName);
        const { artifacts } = await agentGen.collectAgentArtifacts(flowDir, selectedModules);
      ...
      if (!artifact_types || artifact_types.includes('workflows')) {
        const workflowGen = new WorkflowCommandGenerator(this.flowFolderName);
        const { artifacts } = await workflowGen.collectWorkflowArtifacts(flowDir);
      ...
      if (!artifact_types || artifact_types.includes('tasks') || artifact_types.includes('tools')) {
        const taskToolGen = new TaskToolCommandGenerator(this.flowFolderName);
        const { artifacts } = await taskToolGen.collectTaskToolArtifacts(flowDir);
    ...
    if (config.skill_format) {
      results.skills = await this.installVerbatimSkills(projectDir, flowDir, targetPath, config);
    }
   ...
   * @returns {Promise<Object>} Installation result
   */
  async installToMultipleTargets(projectDir, flowDir, targets, options) {
    const allResults = { agents: 0, workflows: 0, tasks: 0, tools: 0, skills: 0 };

    for (const target of targets) {
      const result = await this.installToTarget(projectDir, flowDir, target, options);
      if (result.success) {
...
1. LOAD the FULL agent file from {project-root}/{{flowFolderName}}/{{path}}
...
LOAD and execute from: {project-root}/{{flowFolderName}}/{{path}}
`;
  }
  ...
  renderTemplate(template, artifact) {
    ...
    // Replace _flow placeholder with actual folder name BEFORE inserting paths,
    // so that paths containing '_flow' are not corrupted by the blanket replacement.
    let rendered = template.replaceAll('_flow', this.flowFolderName);

    // Replace {{flowFolderName}} placeholder if present
    rendered = rendered.replaceAll('{{flowFolderName}}', this.flowFolderName);
  ...
  async installCustomAgentLauncher(projectDir, agentName, agentPath, metadata) {
    ...
    // The default-agent template already includes the _flow/ prefix before {{path}},
    // but agentPath is relative to project root (e.g. "_flow/custom/agents/fred.md").
    // Strip the flowFolderName prefix so the template doesn't produce a double path.
    const flowPrefix = this.flowFolderName + '/';
    const normalizedPath = agentPath.startsWith(flowPrefix) ? agentPath.slice(flowPrefix.length) : agentPath;
  ...
  generateFilename(artifact, artifactType, extension = '.md') {
    ...
    // If using default markdown, preserve the flow-agent- prefix for agents
    if (extension === '.md') {
      return baseName;
    }
   ...
   */
  async installVerbatimSkills(projectDir, flowDir, targetPath, config) {
    const flowFolderName = path.basename(flowDir);
    const flowPrefix = flowFolderName + '/';
    const csvPath = path.join(flowDir, '_config', 'skill-manifest.csv');
    ...
    for (const record of records) {
      ...
      // path is like "_flow/bmm/workflows/flow-quick-flow/flow-quick-dev-new-preview/SKILL.md"
      // Strip flowFolderName prefix and join with flowDir, then get dirname
      const relativePath = record.path.startsWith(flowPrefix) ? record.path.slice(flowPrefix.length) : record.path;
      const sourceFile = path.join(flowDir, relativePath);
    ...
    // Post-install cleanup: remove _flow/ directories for skills with install_to_flow === "false"
    for (const record of records) {
      if (record.install_to_flow === 'false') {
        const relativePath = record.path.startsWith(flowPrefix) ? record.path.slice(flowPrefix.length) : record.path;
        const sourceFile = path.join(flowDir, relativePath);
        const sourceDir = path.dirname(sourceFile);
  ...
  async cleanup(projectDir, options = {}) {
    ...
    // Strip FLOW markers from copilot-instructions.md if present
    if (this.name === 'github-copilot') {
      await this.cleanupCopilotInstructions(projectDir, options);
    }

    // Strip FLOW modes from .kilocodemodes if present
    if (this.name === 'kilo') {
      await this.cleanupKiloModes(projectDir, options);
    }

    // Strip FLOW entries from .rovodev/prompts.yml if present
  ...
  /**
   * Warn about stale FLOW files in a global legacy directory (never auto-deletes)
  ...
  async warnGlobalLegacy(legacyDir, options = {}) {
    try {
      ...
      const flowFiles = entries.filter((e) => typeof e === 'string' && e.startsWith('flow'));

      if (flowFiles.length > 0 && !options.silent) {
        await prompts.log.warn(`Found ${flowFiles.length} stale FLOW file(s) in ${expanded}. Remove manually: rm ${expanded}/flow-*`);
      }
    } catch {
  ...
  async cleanupTarget(projectDir, targetDir, options = {}) {
    ...
    for (const entry of entries) {
      ...
      if (entry.startsWith('flow') && !entry.startsWith('flow-os-')) {
    ...
    if (removedCount > 0 && !options.silent) {
      await prompts.log.message(`  Cleaned ${removedCount} FLOW files from ${targetDir}`);
    }
  ...
  /**
   * Strip FLOW-owned content from .github/copilot-instructions.md.
   * The old custom installer injected content between <!-- FLOW:START --> and <!-- FLOW:END --> markers.
  ...
  async cleanupCopilotInstructions(projectDir, options = {}) {
    ...
    try {
      ...
      const startIdx = content.indexOf('<!-- FLOW:START -->');
      const endIdx = content.indexOf('<!-- FLOW:END -->');

      if (startIdx === -1 || endIdx === -1 || endIdx <= startIdx) return;

      const cleaned = content.slice(0, startIdx) + content.slice(endIdx + '<!-- FLOW:END -->'.length);
      ...
      if (!options.silent) await prompts.log.message('  Cleaned FLOW markers from copilot-instructions.md');
    } catch {
      if (!options.silent) await prompts.log.warn('  Warning: Could not clean FLOW markers from copilot-instructions.md');
    }
  }

  /**
   * Strip FLOW-owned modes from .kilocodemodes.
   * The old custom kilo.js installer added modes with slug starting with 'flow-'.
   * Parses YAML, filters out FLOW modes, rewrites. Leaves file as-is on parse failure.
   */
  async cleanupKiloModes(projectDir, options = {}) {
    ...
    config.customModes = config.customModes.filter((mode) => mode && (!mode.slug || !mode.slug.startsWith('flow-')));
    ...
    if (removedCount > 0) {
      try {
        ...
        if (!options.silent) await prompts.log.message(`  Removed ${removedCount} FLOW modes from .kilocodemodes`);
      } catch {
  ...
  /**
   * Strip FLOW-owned entries from .rovodev/prompts.yml.
   * The old custom rovodev.js installer registered workflows in prompts.yml.
   * Parses YAML, filters out entries with name starting with 'flow-', rewrites.
  ...
  async cleanupRovoDevPrompts(projectDir, options = {}) {
    ...
    config.prompts = config.prompts.filter((entry) => entry && (!entry.name || !entry.name.startsWith('flow-')));
    ...
    if (removedCount > 0) {
      try {
        ...
        if (!options.silent) await prompts.log.message(`  Removed ${removedCount} FLOW entries from prompts.yml`);
      } catch {
  ...
  /**
   * Check ancestor directories for existing FLOW files in the same target_dir.
    ...
    while (current !== root && current.length > root.length) {
      ...
      try {
        if (await fs.pathExists(candidatePath)) {
          ...
          const hasFlow = entries.some(
            (e) => typeof e === 'string' && e.toLowerCase().startsWith('flow') && !e.toLowerCase().startsWith('flow-os-'),
          );
          if (hasFlow) {
