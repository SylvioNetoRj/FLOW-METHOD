...
class ConfigCollector {
   ...
   * @returns {Promise<string>} Path to flow directory
   */
  async findFlowDir(projectDir) {
    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      // Project doesn't exist yet, return default
      return path.join(projectDir, 'flow');
    }
  ...
  /**
   * Detect the existing FLOW folder name in a project
   * @param {string} projectDir - Project directory
   * @returns {Promise<string|null>} Folder name (just the name, not full path) or null if not found
   */
  async detectExistingFlowFolder(projectDir) {
    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      return null;
    }
  ...
  async loadExistingConfig(projectDir) {
    ...
    // Find the actual flow directory (handles custom folder names)
    ...
    // Dynamically discover all installed modules by scanning flow directory
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const moduleConfigPath = path.join(flowDir, entry.name, 'config.yaml');
