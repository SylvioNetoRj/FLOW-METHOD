...
/**
 * FLOW Method CLI - Direct execution wrapper for npx
...
if (isNpxExecution) {
  ...
  const flowCliPath = path.join(__dirname, 'cli', 'flow-cli.js');

  if (!fs.existsSync(flowCliPath)) {
    console.error('Error: Could not find flow-cli.js at', flowCliPath);
  ...
  try {
    ...
    execSync(`node "${flowCliPath}" ${args.join(' ')}`, {
      stdio: 'inherit',
