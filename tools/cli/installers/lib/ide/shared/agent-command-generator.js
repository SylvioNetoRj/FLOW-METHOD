...
const { toColonPath, toDashPath, customAgentColonName, customAgentDashName, FLOW_FOLDER_NAME } = require('./path-utils');
...
class AgentCommandGenerator {
   ...
   */
  async collectAgentArtifacts(flowDir, selectedModules = []) {
    const { getAgentsFromFlow } = require('./flow-artifacts');

    // Get agents from INSTALLED flow/ directory
    const agents = await getAgentsFromFlow(flowDir, selectedModules);
    ...
    for (const agent of agents) {
      ...
      // Remove _flow/ prefix if present to get relative path from project root
      // Handle both absolute paths (/path/to/_flow/...) and relative paths (_flow/...)
      if (agentRelPath.includes('_flow/')) {
        const parts = agentRelPath.split(/_flow\//);
        if (parts.length > 1) {
  ...
  async generateLauncherContent(agent) {
    ...
    return template
      ...
      .replaceAll('_flow', this.flowFolderName)
      .replaceAll('_flow', '_flow');
  }
   ...
   * Creates flat files like: flow_bmm_pm.md
   *
   * @param {string} baseCommandsDir - Base commands directory for the IDE
   * @param {Array} artifacts - Agent launcher artifacts
   * @returns {number} Count of launchers written
  ...
  async writeColonArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'agent-launcher') {
        // Convert relativePath to underscore format: bmm/agents/pm.md → flow_bmm_pm.md
   ...
   * Creates flat files like: flow-agent-bmm-pm.md
   *
   * The flow-agent- prefix distinguishes agents from workflows/tasks/tools.
   *
   * @param {string} baseCommandsDir - Base commands directory for the IDE
   * @param {Array} artifacts - Agent launcher artifacts
   * @returns {number} Count of launchers written
  ...
  async writeDashArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'agent-launcher') {
        // Convert relativePath to dash format: bmm/agents/pm.md → flow-agent-bmm-pm.md
