---
title: "非交互式安装"
description: 使用命令行标志安装 FLOW，适用于 CI/CD 流水线和自动化部署
sidebar:
  order: 2
---

使用命令行标志以非交互方式安装 FLOW。这适用于：
...
| `--output-folder <path>` | 输出文件夹路径 | _flow-output |
...
查看 [FLOW 注册表](https://github.com/flow-code-org) 获取可用的外部模块。
...
:::note[仍然卡住了？]
使用 `--debug` 运行以获取详细输出，尝试交互模式以隔离问题，或在 <https://github.com/SylvioNetoRj/FLOW-METHOD/issues> 报告。
:::
