...
class Detector {
  ...
  async detect(flowDir) {
    ...
    const manifestData = await manifest.read(flowDir);
    if (manifestData) {
    ...
    // Check for core
    const corePath = path.join(flowDir, 'core');
  ...
  /**
   * Detect legacy installation (_flow-method, .bmm, .cis)
  ...
  async detectLegacy(projectDir) {
    ...
    // Check for legacy core (_flow-method)
    const legacyCorePath = path.join(projectDir, '_flow-method');
    ...
    for (const entry of entries) {
      if (
        ...
        // Check if it's likely a FLOW module
  ...
  async checkMigrationNeeded(projectDir) {
    const flowDir = path.join(projectDir, 'flow');
    const current = await this.detect(flowDir);
  ...
  /**
   * Detect legacy FLOW v4 .flow-method folder
   * @param {string} projectDir - Project directory to check
  ...
  async detectLegacyV4(projectDir) {
    ...
    // Check for .flow-method folder
    const flowMethodPath = path.join(projectDir, '.flow-method');
    if (await fs.pathExists(flowMethodPath)) {
      offenders.push(flowMethodPath);
    }
