...
class CustomHandler {
   ...
   */
  async install(customPath, flowDir, config, fileTrackingCallback = null) {
    const results = {
      agentsInstalled: 0,
    ...
    try {
      // Create custom directories in flow
      const flowCustomDir = path.join(flowDir, 'custom');
      const flowAgentsDir = path.join(flowCustomDir, 'agents');
      const flowWorkflowsDir = path.join(flowCustomDir, 'workflows');

      await fs.ensureDir(flowCustomDir);
      await fs.ensureDir(flowAgentsDir);
      await fs.ensureDir(flowWorkflowsDir);

      // Process agents - compile and copy agents
      const agentsDir = path.join(customPath, 'agents');
      if (await fs.pathExists(agentsDir)) {
        await this.compileAndCopyAgents(agentsDir, flowAgentsDir, flowDir, config, fileTrackingCallback, results);
      ...
      if (await fs.pathExists(workflowsDir)) {
        await this.copyDirectory(workflowsDir, flowWorkflowsDir, results, fileTrackingCallback, config);
      ...
      for (const entry of entries) {
        if (entry.isFile() && entry.name !== 'custom.yaml' && !entry.name.startsWith('.') && !entry.name.endsWith('.md')) {
          ...
          const targetPath = path.join(flowCustomDir, entry.name);
   ...
   * @param {Object} results - Results object to update
   */
  async compileAndCopyAgents(sourceAgentsPath, targetAgentsPath, flowDir, config, fileTrackingCallback, results) {
    ...
    for (const agentFile of agentFiles) {
      ...
      // Use the actual flowDir if available (for when installing to temp dir)
      const actualFlowDir = config._flowDir || flowDir;
      const customizePath = path.join(actualFlowDir, '_config', 'agents', `custom-${agentName}.customize.yaml`);
