...
const { FLOW_FOLDER_NAME } = require('../ide/shared/path-utils');
...
class Installer {
  constructor() {
    ...
    this.flowFolderName = FLOW_FOLDER_NAME;
  }

  /**
   * Find the flow installation directory in a project
   * Always uses the standard _flow folder name
   * Also checks for legacy _cfg folder for migration
   * @param {string} projectDir - Project directory
   * @returns {Promise<Object>} { flowDir: string, hasLegacyCfg: boolean }
   */
  async findFlowDir(projectDir) {
    const flowDir = path.join(projectDir, FLOW_FOLDER_NAME);

    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      // Project doesn't exist yet, return default
      return { flowDir, hasLegacyCfg: false };
    }

    // Check for legacy _cfg folder if flow directory exists
    let hasLegacyCfg = false;
    if (await fs.pathExists(flowDir)) {
      const legacyCfgPath = path.join(flowDir, '_cfg');
      if (await fs.pathExists(legacyCfgPath)) {
        hasLegacyCfg = true;
      }
    }

    return { flowDir, hasLegacyCfg };
  }

  /**
   * @function copyFileWithPlaceholderReplacement
   * @intent Copy files from FLOW source to installation directory with dynamic content transformation
   * @why Enables installation-time customization: _flow replacement
   * @param {string} sourcePath - Absolute path to source file in FLOW repository
   * @param {string} targetPath - Absolute path to destination file in user's project
   * @param {string} flowFolderName - User's chosen flow folder name (default: 'flow')
  ...
  async collectToolConfigurations(
    ...
    // During full reinstall, use the saved previous IDEs since flow dir was deleted
    ...
    if (isFullReinstall) {
      ...
      const existingInstall = await detector.detect(flowDir);
      previouslyConfiguredIdes = existingInstall.ides || [];
    }

    // Load saved IDE configurations for already-configured IDEs
    const savedIdeConfigs = await this.ideConfigManager.loadAllIdeConfigs(flowDir);
  ...
  async install(originalConfig) {
    ...
    if (!hasCoreConfig) {
      ...
      await CLIUtils.displaySection('FLOW™  Installation', 'Version ' + require(path.join(getProjectRoot(), 'package.json')).version);
    }
      ...
      if (config._isUpdate && config._existingInstall && config._existingInstall.customModules) {
        for (const customModule of config._existingInstall.customModules) {
          ...
          if (absoluteSourcePath && absoluteSourcePath.startsWith('_config')) {
            ...
            absoluteSourcePath = path.join(flowDir, absoluteSourcePath);
          }
          // If no sourcePath but we have relativePath, convert it
          else if (!absoluteSourcePath && customModule.relativePath) {
            // relativePath is relative to the project root (parent of flow dir)
    ...
    // Set flow folder name on module manager and IDE manager for placeholder replacement
    this.moduleManager.setFlowFolderName(FLOW_FOLDER_NAME);
    this.moduleManager.setCoreConfig(moduleConfigs.core || {});
    this.moduleManager.setCustomModulePaths(customModulePaths);
    this.ideManager.setFlowFolderName(FLOW_FOLDER_NAME);
    ...
    try {
      ...
      const existingInstall = await this.detector.detect(flowDir);
      ...
      if (existingInstall.installed && !config.force && !config._quickUpdate) {
        ...
        if (config.actionType === 'update') {
          ...
          await prompts.log.warn('Existing FLOW installation detected');
          await prompts.log.message(`  Location: ${flowDir}`);
        ...
        if (action === 'update') {
          ...
          if (modulesToRemove.length > 0) {
            if (config.skipPrompts) {
              ...
              for (const moduleId of modulesToRemove) {
                ...
                const modulePath = path.join(flowDir, moduleId);
                await prompts.log.error(`  - ${displayName} (${modulePath})`);
              }

              const confirmRemoval = await prompts.confirm({
                message: `Remove ${modulesToRemove.length} module(s) from FLOW installation?`,
                default: false,
              });
          ...
          const existingFilesManifest = await this.readFilesManifest(flowDir);
          const { customFiles, modifiedFiles } = await this.detectCustomFiles(flowDir, existingFilesManifest);
          ...
          const coreConfigPath = path.join(flowDir, 'core', 'config.yaml');
          ...
          const cacheDir = path.join(flowDir, '_config', 'custom');
          ...
          if (customFiles.length > 0) {
            const tempBackupDir = path.join(projectDir, '_flow-custom-backup-temp');
            await fs.ensureDir(tempBackupDir);

            spinner.start(`Backing up ${customFiles.length} custom files...`);
            for (const customFile of customFiles) {
              const relativePath = path.relative(flowDir, customFile);
          ...
          if (modifiedFiles.length > 0) {
            const tempModifiedBackupDir = path.join(projectDir, '_flow-modified-backup-temp');
            await fs.ensureDir(tempModifiedBackupDir);

            spinner.start(`Backing up ${modifiedFiles.length} modified files...`);
            for (const modifiedFile of modifiedFiles) {
              const relativePath = path.relative(flowDir, modifiedFile.path);
      ...
      if (config.ides && config.ides.length > 0) {
        ...
        if (suspendedIdes.length > 0 && suspendedIdes.length === config.ides.length) {
          ...
          throw new Error(
            `All selected tool(s) are suspended: ${suspendedIdes.join(', ')}. Installation aborted to prevent upgrading _flow/ without a working IDE configuration.`,
          );
        }
      }
      ...
      if (config._isUpdate && config._existingInstall) {
        ...
        if (idesToRemove.length > 0) {
          if (config.skipPrompts) {
            ...
            const confirmRemoval = await prompts.confirm({
              message: `Remove FLOW configuration for ${idesToRemove.length} IDE(s)?`,
              default: false,
            });
            ...
            if (confirmRemoval) {
              ...
              for (const ide of idesToRemove) {
                try {
                  ...
                  await this.ideConfigManager.deleteIdeConfig(flowDir, ide);
      ...
      await this.createDirectoryStructure(flowDir);
      ...
      if (customModulePaths && customModulePaths.size > 0) {
        ...
        const customCache = new CustomModuleCache(flowDir);
      ...
      // Custom modules are already installed in _flow and don't need dependency resolution from source
      ...
      if (config.installCore) {
        installTasks.push({
          title: isQuickUpdate ? 'Updating FLOW core' : 'Installing FLOW core',
          task: async (message) => {
            await this.installCoreWithDependencies(flowDir, { core: {} });
            addResult('Core', 'ok', isQuickUpdate ? 'updated' : 'installed');
            await this.generateModuleConfigs(flowDir, { core: config.coreConfig || {} });
          ...
          task: async (message) => {
            ...
            for (const moduleName of allModules) {
              ...
              if (!isCustomModule && config._customModuleSources && config._customModuleSources.has(moduleName)) {
                ...
                if (customInfo.sourcePath && !customInfo.path) {
                  customInfo.path = path.isAbsolute(customInfo.sourcePath)
                    ...
                    : path.join(flowDir, customInfo.sourcePath);
                }
              }
              ...
              if (isCustomModule && customInfo) {
                ...
                await this.generateModuleConfigs(flowDir, {
                ...
                if (moduleName === 'core') {
                  await this.installCoreWithDependencies(flowDir, resolution.byModule[moduleName]);
                } else {
                  await this.installModuleWithDependencies(moduleName, flowDir, resolution.byModule[moduleName]);
                }
              }
            ...
            for (const [module, files] of Object.entries(resolution.byModule)) {
              if (!allModules.includes(module) && module !== 'core') {
                ...
                if (totalFiles > 0) {
                  ...
                  await this.installPartialModule(module, flowDir, files);
                }
              }
            }
      ...
      installTasks.push({
        ...
        task: async (message) => {
          ...
          const verboseMode = process.env.FLOW_VERBOSE_INSTALL === 'true' || config.verbose;
          const moduleLogger = {
          ...
          if (config.installCore || resolution.byModule.core) {
            const result = await this.moduleManager.createModuleDirectories('core', flowDir, {
          ...
          if (config.modules && config.modules.length > 0) {
            for (const moduleName of config.modules) {
              ...
              const result = await this.moduleManager.createModuleDirectories(moduleName, flowDir, {
                installedIDEs: config.ides || [],
      ...
      const configTask = {
        ...
        task: async (message) => {
          ...
          await this.generateModuleConfigs(flowDir, moduleConfigs);
          addResult('Configurations', 'ok', 'generated');

          // Pre-register manifest files
          const cfgDir = path.join(flowDir, '_config');
          ...
          const manifestStats = await manifestGen.generateManifests(flowDir, allModulesForManifest, [...this.installedFiles], {
            ides: config.ides || [],
          ...
          await this.mergeModuleHelpCatalogs(flowDir);
        ...
        if (validIdes.length === 0) {
          ...
          try {
            for (const ide of validIdes) {
              ...
              try {
                const setupResult = await this.ideManager.setup(ide, projectDir, flowDir, {
                ...
                if (ideConfigurations[ide] && !ideConfigurations[ide]._alreadyConfigured) {
                  await this.ideConfigManager.saveIdeConfig(flowDir, ide, ideConfigurations[ide]);
                }
          ...
          task: async (message) => {
            ...
            if (config._customFiles && config._customFiles.length > 0) {
              ...
              for (const originalPath of config._customFiles) {
                const relativePath = path.relative(flowDir, originalPath);
   ...
   * @param {Object} context - {flowDir, modules, ides, customFiles, modifiedFiles}
   */
  async renderInstallSummary(results, context = {}) {
    ...
    lines.push('');
    if (context.flowDir) {
      lines.push(`  Installed to: ${color.dim(context.flowDir)}`);
    }
  ...
  async update(config) {
    ...
    try {
      const projectDir = path.resolve(config.directory);
      const { flowDir } = await this.findFlowDir(projectDir);
      const existingInstall = await this.detector.detect(flowDir);

      if (!existingInstall.installed) {
        spinner.stop('No FLOW installation found');
        throw new Error(`No FLOW installation found at ${flowDir}`);
      }
      ...
      if (existingInstall.hasCore) {
        ...
        await this.updateCore(flowDir, config.force);
      }

      for (const module of existingInstall.modules) {
        spinner.message(`Updating module: ${module.id}...`);
        await this.moduleManager.update(module.id, flowDir, config.force, { installer: this });
      }

      // Update manifest
      spinner.message('Updating manifest...');
      await this.manifest.update(flowDir, {
        version: newVersion,
  ...
  async getStatus(directory) {
    ...
    const { flowDir } = await this.findFlowDir(projectDir);
    return await this.detector.detect(flowDir);
  }
  ...
  /**
   * Uninstall FLOW with selective removal options
   * @param {string} directory - Project directory
   * @param {Object} options - Uninstall options
   * @param {boolean} [options.removeModules=true] - Remove _flow/ directory
  ...
  async uninstall(directory, options = {}) {
    ...
    const existingInstall = await this.detector.detect(flowDir);
    const outputFolder = await this._readOutputFolder(flowDir);

    const removed = { modules: false, ideConfigs: false, outputFolder: false };

    // 2. IDE CLEANUP (before _flow/ deletion so configs are accessible)
    ...
    // 4. FLOW DIRECTORY (last, after everything that needs it)
   ...
   * Resolves flowDir internally from projectDir
   * @param {string} projectDir - Project directory
   * @returns {string} Output folder name (relative, default: '_flow-output')
   */
  async getOutputFolder(projectDir) {
    const { flowDir } = await this.findFlowDir(projectDir);
    return this._readOutputFolder(flowDir);
  }

  /**
   * Read the output_folder setting from module config files
   * Checks bmm/config.yaml first, then other module configs
   * @param {string} flowDir - FLOW installation directory
   * @returns {string} Output folder path or default
   */
  async _readOutputFolder(flowDir) {
    const yaml = require('yaml');

    // Check bmm/config.yaml first (most common)
    const bmmConfigPath = path.join(flowDir, 'bmm', 'config.yaml');
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const configPath = path.join(flowDir, entry.name, 'config.yaml');
   ...
   */
  /**
   * Merge all module-help.csv files into a single flow-help.csv
   * Scans all installed modules for module-help.csv and merges them
   * Enriches agent info from agent-manifest.csv
   * Output is written to _flow/_config/flow-help.csv
   * @param {string} flowDir - FLOW installation directory
   */
  async mergeModuleHelpCatalogs(flowDir) {
    const allRows = [];
    const headerRow =
      'module,phase,name,code,sequence,workflow-file,command,required,agent-name,agent-command,agent-display-name,agent-title,options,description,output-location,outputs';

    // Load agent manifest for agent info lookup
    const agentManifestPath = path.join(flowDir, '_config', 'agent-manifest.csv');
    ...
    if (await fs.pathExists(agentManifestPath)) {
      ...
      for (const line of lines) {
        ...
        if (cols.length >= 4) {
          ...
          // Build agent command: flow:module:agent:name
          const agentCommand = module ? `flow:${module}:agent:${agentName}` : `flow:agent:${agentName}`;
    ...
    for (const moduleName of installedModules) {
      const modulePath = path.join(flowDir, moduleName);
    ...
    for (const [moduleName, modulePath] of modulePaths) {
      ...
      if (await fs.pathExists(helpFilePath)) {
        try {
          ...
          if (process.env.FLOW_VERBOSE_INSTALL === 'true') {
    ...
    const outputDir = path.join(flowDir, '_config');
    await fs.ensureDir(outputDir);
    const outputPath = path.join(outputDir, 'flow-help.csv');
    ...
    if (process.env.FLOW_VERBOSE_INSTALL === 'true') {
      await prompts.log.message(`  Generated flow-help.csv: ${allRows.length} workflows`);
    }
  }
  ...
  async createDirectoryStructure(flowDir) {
    await fs.ensureDir(flowDir);
    await fs.ensureDir(path.join(flowDir, '_config'));
    await fs.ensureDir(path.join(flowDir, '_config', 'agents'));
    await fs.ensureDir(path.join(flowDir, '_config', 'custom'));
  }
   ...
   */
  async generateModuleConfigs(flowDir, moduleConfigs) {
    const yaml = require('yaml');
...
`;
   ...
   */
  async installCoreWithDependencies(flowDir, coreFiles) {
    const sourcePath = getModulePath('core');
    const targetPath = path.join(flowDir, 'core');
    await this.installCore(flowDir);
  }
   ...
   */
  async installModuleWithDependencies(moduleName, flowDir, moduleFiles) {
   ...
   */
  async installPartialModule(moduleName, flowDir, files) {
    const sourceBase = getModulePath(moduleName);
    const targetBase = path.join(flowDir, moduleName);
  ...
  async installCore(flowDir) {
    ...
    await moduleManager.compileModuleAgents(sourcePath, targetPath, 'core', flowDir, this);
   ...
   * @param {string} modulePath - Path to module in flow/ installation
   * @param {string} moduleName - Module name
   */
  async processAgentFiles(modulePath, moduleName) {
    ...
    // Determine project directory (parent of flow/ directory)
    const flowDir = path.dirname(modulePath);
    const cfgAgentsDir = path.join(flowDir, '_config', 'agents');
   ...
   * Private: Update core
   */
  async updateCore(flowDir, force = false) {
    const sourcePath = getModulePath('core');
    const targetPath = path.join(flowDir, 'core');
  ...
  async quickUpdate(config) {
    ...
    try {
      ...
      if (!(await fs.pathExists(flowDir))) {
        spinner.stop('No FLOW installation found');
        throw new Error(`FLOW not installed at ${flowDir}. Use regular install for first-time setup.`);
      }
      ...
      const projectRoot = path.dirname(flowDir);
  ...
  async compileAgents(config) {
    ...
    try {
      ...
      const moduleManager = new ModuleManager();
      moduleManager.setFlowFolderName(path.basename(flowDir));
      ...
      for (const moduleId of installedModules) {
        ...
        const targetPath = path.join(flowDir, moduleId);

        // Compile agents for this module
        await moduleManager.compileModuleAgents(sourcePath, targetPath, moduleId, flowDir, this);
  ...
  /**
   * Handle legacy FLOW v4 detection with simple warning
  ...
  async handleLegacyV4Migration(_projectDir, _legacyV4) {
    await prompts.note(
      'Found .flow-method folder from FLOW v4 installation.\n\n' +
        'Before continuing with installation, we recommend:\n' +
        '  1. Remove the .flow-method folder, OR\n' +
        '  2. Back it up by renaming it to another name (e.g., flow-method-backup)\n\n' +
        'If your v4 installation set up rules or commands, you should remove those as well.',
      'Legacy FLOW v4 detected',
    );
    ...
    if (proceed === 'exit') {
      await prompts.log.info('Please remove the .flow-method folder and any v4 rules/commands, then run the installer again.');
   ...
   */
  async readFilesManifest(flowDir) {
    const filesManifestPath = path.join(flowDir, '_config', 'files-manifest.csv');
   ...
   */
  async detectCustomFiles(flowDir, existingFilesManifest) {
    const customFiles = [];
    const modifiedFiles = [];

    // Memory is always in _flow/_memory
    const flowMemoryPath = '_memory';
    ...
    for (const fileEntry of existingFilesManifest) {
      if (fileEntry.path) {
        const absolutePath = path.join(flowDir, fileEntry.path);
    ...
    // Recursively scan flowDir for all files
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            const relativePath = path.relative(flowDir, fullPath);
            ...
            if (relativePath.startsWith('_config/') || relativePath.startsWith('_config\\')) {
              ...
              if (relativePath.includes('/agents/') && fileName.endsWith('.customize.yaml')) {
                ...
                const manifestPath = path.join(flowDir, '_config', 'manifest.yaml');
            ...
            if (relativePath.startsWith(flowMemoryPath + '/') && path.dirname(relativePath).includes('-sidecar')) {
              continue;
            }
   ...
   * @param {string} flowDir - FLOW directory
   ...
   */
  async handleMissingCustomSources(customModuleSources, flowDir, projectRoot, operation, installedModules, skipPrompts = false) {
    ...
    for (const missing of customModulesWithMissingSources) {
      ...
      switch (action) {
        case 'update': {
          ...
          await this.manifest.addCustomModule(flowDir, missing.info);
        ...
        case 'remove': {
          ...
          if (confirmDelete) {
            ...
            if (typedConfirm === 'DELETE') {
              ...
              const modulePath = path.join(flowDir, missing.id);
              if (await fs.pathExists(modulePath)) {
              ...
              await this.manifest.removeModule(flowDir, missing.id);
              await this.manifest.removeCustomModule(flowDir, missing.id);
