...
const { FLOW_FOLDER_NAME } = require('./path-utils');
...
class WorkflowCommandGenerator {
  ...
  async collectWorkflowArtifacts(flowDir) {
    const workflows = await this.loadWorkflowManifest(flowDir);
    ...
    for (const workflow of allWorkflows) {
      ...
      if (workflowRelPath.includes('_flow/')) {
        const parts = workflowRelPath.split(/_flow\//);
        if (parts.length > 1) {
...
`;
  ...
  transformWorkflowPath(workflowPath) {
    ...
    if (workflowPath.includes('/src/bmm/')) {
      ...
      if (match) {
        transformed = `{project-root}/${this.flowFolderName}/bmm/${match[1]}`;
      }
    } else if (workflowPath.includes('/src/core/')) {
      const match = workflowPath.match(/\/src\/core\/(.+)/);
      if (match) {
        transformed = `{project-root}/${this.flowFolderName}/core/${match[1]}`;
      }
    }
  ...
  async loadWorkflowManifest(flowDir) {
    const manifestPath = path.join(flowDir, '_config', 'workflow-manifest.csv');
