...
const { FLOW_FOLDER_NAME } = require('./shared/path-utils');
...
class BaseIdeSetup {
  ...
  async cleanup(projectDir, options = {}) {
    ...
    if (this.configDir) {
      ...
      if (await fs.pathExists(configPath)) {
        const flowRulesPath = path.join(configPath, FLOW_FOLDER_NAME);
        if (await fs.pathExists(flowRulesPath)) {
          await fs.remove(flowRulesPath);
          if (!options.silent) await prompts.log.message(`Removed ${this.name} FLOW configuration`);
        }
      }
    }
  }
  ...
  /**
   * Get list of agents from FLOW installation
   * @param {string} flowDir - FLOW installation directory
   * @returns {Array} List of agent files
   */
  async getAgents(flowDir) {
    const agents = [];

    // Get core agents
    const coreAgentsPath = path.join(flowDir, 'core', 'agents');
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_config' && entry.name !== 'agents') {
        const moduleAgentsPath = path.join(flowDir, entry.name, 'agents');
    ...
    const standaloneAgentsDir = path.join(flowDir, 'agents');
  ...
  async getTasks(flowDir, standaloneOnly = false) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_config' && entry.name !== 'agents') {
        const moduleTasksPath = path.join(flowDir, entry.name, 'tasks');
  ...
  async getTools(flowDir, standaloneOnly = false) {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_config' && entry.name !== 'agents') {
        const moduleToolsPath = path.join(flowDir, entry.name, 'tools');
  ...
  /**
   * Get list of workflows from FLOW installation
   * @param {string} flowDir - FLOW installation directory
   * @param {boolean} standaloneOnly - If true, only return standalone workflows
   * @returns {Array} List of workflow files
   */
  async getWorkflows(flowDir, standaloneOnly = false) {
    const workflows = [];

    // Get core workflows
    const coreWorkflowsPath = path.join(flowDir, 'core', 'workflows');
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name !== 'core' && entry.name !== '_config' && entry.name !== 'agents') {
        const moduleWorkflowsPath = path.join(flowDir, entry.name, 'workflows');
  ...
  /**
   * Write file with content (replaces _flow placeholder)
   * @param {string} filePath - File path
   * @param {string} content - File content
   */
  async writeFile(filePath, content) {
    // Replace _flow placeholder if present
    if (typeof content === 'string' && content.includes('_flow')) {
      content = content.replaceAll('_flow', this.flowFolderName);
    }

    // Replace escape sequence _flow with literal _flow
    if (typeof content === 'string' && content.includes('_flow')) {
      content = content.replaceAll('_flow', '_flow');
    }
  ...
  /**
   * Copy file from source to destination (replaces _flow placeholder in text files)
  ...
  async copyFile(source, dest) {
    ...
    if (textExtensions.includes(ext)) {
      try {
        ...
        // Replace _flow placeholder with actual folder name
        if (content.includes('_flow')) {
          content = content.replaceAll('_flow', this.flowFolderName);
        }
   ...
   * Example: 'module/agents/name.md' -> 'flow-module-agents-name.md'
   ...
   * @returns {string} Flattened filename with 'flow-' prefix
   */
   ...
   */
  async createAgentConfig(flowDir, agent) {
    const agentConfigDir = path.join(flowDir, '_config', 'agents');
