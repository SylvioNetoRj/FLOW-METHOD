...
class UI {
  ...
  async promptInstall(options = {}) {
    ...
    // Preflight: Check for legacy FLOW v4 footprints immediately after getting directory
    ...
    let hasLegacyFlowFolder = false;
    let flowDir = null;
    let legacyFlowPath = null;

    // First check for legacy .flow folder (instead of _flow)
    ...
    if (await fs.pathExists(confirmedDirectory)) {
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && (entry.name === '.flow' || entry.name === 'flow')) {
          hasLegacyFlowFolder = true;
          legacyFlowPath = path.join(confirmedDirectory, entry.name);
          flowDir = legacyFlowPath;

          // Check if it has _cfg folder
          const cfgPath = path.join(legacyFlowPath, '_cfg');
    ...
    // If no .flow or flow found, check for current installations _flow
    if (!hasLegacyFlowFolder) {
      const flowResult = await installer.findFlowDir(confirmedDirectory);
      flowDir = flowResult.flowDir;
      hasLegacyCfg = flowResult.hasLegacyCfg;
    }

    // Handle legacy .flow or _cfg folder - these are very old (v4 or alpha)
    // Show version warning instead of offering conversion
    if (hasLegacyFlowFolder || hasLegacyCfg) {
      await prompts.log.warn('LEGACY INSTALLATION DETECTED');
      await prompts.note(
        'Found a ".flow"/"flow" folder, or a legacy "_cfg" folder under the flow folder -\n' +
          'this is from an old FLOW version that is out of date for automatic upgrade,\n' +
          ...
          '  1. Delete your current FLOW installation folder (.flow or flow)\n' +
          '  2. Run a fresh installation\n\n' +
          'If you do not want to start fresh, you can attempt to proceed beyond this\n' +
          'point IF you have ensured the flow folder is named _flow, and under it there\n' +
          'is a _config folder. If you have a folder under your flow folder named _cfg,\n' +
      ...
      if (proceed === 'cancel') {
        await prompts.note('1. Delete the existing flow folder in your project\n' + "2. Run 'flow install' again", 'To do a fresh install');
        process.exit(0);
        return;
      }
      ...
      try {
        // Handle .flow folder
        if (hasLegacyFlowFolder) {
          const newFlowPath = path.join(confirmedDirectory, '_flow');
          await fs.move(legacyFlowPath, newFlowPath);
          flowDir = newFlowPath;
          s.stop(`Renamed "${path.basename(legacyFlowPath)}" to "_flow"`);
        }

        // Handle _cfg folder (either from .flow or standalone)
        const cfgPath = path.join(flowDir, '_cfg');
        if (await fs.pathExists(cfgPath)) {
          s.start('Renaming configuration folder...');
          const newCfgPath = path.join(flowDir, '_config');
    ...
    // Check if there's an existing FLOW installation (after any folder renames)
    const hasExistingInstall = await fs.pathExists(flowDir);
    ...
    if (hasExistingInstall) {
      ...
      const { existingInstall, flowDir } = await this.getExistingInstallation(confirmedDirectory);
      const packageJsonPath = path.join(__dirname, '../../../package.json');
      const currentVersion = require(packageJsonPath).version;
      const installedVersion = existingInstall.version || 'unknown';

      // Check if version is pre beta
      const shouldProceed = await this.showLegacyVersionWarning(installedVersion, currentVersion, path.basename(flowDir), options);
      ...
      // Common actions
      choices.push({ name: 'Modify FLOW Installation', value: 'update' });
      ...
      if (actionType === 'update') {
        ...
        if (options.customContent) {
          ...
          if (changeCustomModules) {
            ...
            const { Installer } = require('../installers/lib/core/installer');
            const installer = new Installer();
            const { flowDir } = await installer.findFlowDir(confirmedDirectory);
  ...
  async promptToolSelection(projectDir, options = {}) {
    // Check for existing configured IDEs - use findFlowDir to detect custom folder names
    const { Detector } = require('../installers/lib/core/detector');
    const { Installer } = require('../installers/lib/core/installer');
    const detector = new Detector();
    const installer = new Installer();
    const flowResult = await installer.findFlowDir(projectDir || process.cwd());
    const flowDir = flowResult.flowDir;
   ...
   * @returns {Object} Object with existingInstall, installedModuleIds, and flowDir
   */
  async getExistingInstallation(directory) {
    ...
    const detector = new Detector();
    const installer = new Installer();
    const flowDirResult = await installer.findFlowDir(directory);
    const flowDir = flowDirResult.flowDir;
    const existingInstall = await detector.detect(flowDir);
    const installedModuleIds = new Set(existingInstall.modules.map((mod) => mod.id));

    return { existingInstall, installedModuleIds, flowDir };
  }
  ...
  async selectAllModules(installedModuleIds = new Set()) {
    ...
    allOptions.push({ label: 'FLOW Core Module', value: 'core', hint: 'Core configuration and shared resources' });
    ...
    // Group 2: FLOW Official Modules (type: flow-org)
    const officialModules = [];
    for (const mod of externalModules) {
      if (mod.type === 'flow-org') {
  ...
  async displayDirectoryInfo(directory) {
    ...
    if (dirExists) {
      ...
      if (stats.isDirectory()) {
        ...
        if (files.length > 0) {
          // Check for any flow installation (any folder with _config/manifest.yaml)
          const { Installer } = require('../installers/lib/core/installer');
          const installer = new Installer();
          const flowResult = await installer.findFlowDir(directory);
          const hasFlowInstall =
            (await fs.pathExists(flowResult.flowDir)) && (await fs.pathExists(path.join(flowResult.flowDir, '_config', 'manifest.yaml')));

          const flowNote = hasFlowInstall ? ` including existing FLOW installation (${path.basename(flowResult.flowDir)})` : '';
          await prompts.log.message(`Directory exists and contains ${files.length} item(s)${flowNote}`);
        } else {
  ...
  async getConfiguredIdes(directory) {
    ...
    const installer = new Installer();
    const flowResult = await installer.findFlowDir(directory);
    const existingInstall = await detector.detect(flowResult.flowDir);
  ...
  async handleCustomModulesInModifyFlow(directory, selectedModules) {
    ...
    const { Installer } = require('../installers/lib/core/installer');
    const installer = new Installer();
    const { flowDir } = await installer.findFlowDir(directory);
   ...
   * @param {string} flowFolderName - Name of the FLOW folder
   * @returns {Promise<boolean>} True if user wants to proceed, false if they cancel
   */
  async showLegacyVersionWarning(installedVersion, currentVersion, flowFolderName, options = {}) {
    ...
    let warningContent;
    if (installedVersion === 'unknown') {
      warningContent = 'Unable to detect your installed FLOW version.\n' + 'This appears to be a legacy or unsupported installation.';
    } else {
      warningContent =
    ...
    warningContent +=
      ...
      '  1. Delete your current FLOW installation folder\n' +
      `     (the "${flowFolderName}/" folder in your project)\n` +
      '  2. Run a fresh installation\n\n' +
      'Benefits of a fresh install:\n' +
      '  \u2022 Cleaner configuration without legacy artifacts\n' +
      '  \u2022 All new features properly configured\n' +
    ...
    if (proceed === 'cancel') {
      await prompts.note(
        `1. Delete the "${flowFolderName}/" folder in your project\n` + "2. Run 'flow install' again",
  ...
  async displayStatus(statusData) {
    const { installation, modules, availableUpdates, flowDir } = statusData;
    ...
    if (availableUpdates.length > 0) {
      ...
      await prompts.log.message('Run \'flow install\' and select "Quick Update" to update');
    } else {
