...
module.exports = {
  command: 'uninstall',
  description: 'Remove FLOW installation from the current project',
  options: [
    ['-y, --yes', 'Remove all FLOW components without prompting (preserves user artifacts)'],
    ...
    try {
      ...
      if (options.directory) {
        ...
        const dirChoice = await prompts.select({
          message: 'Where do you want to uninstall FLOW from?',
          choices: [
      ...
      const { flowDir } = await installer.findFlowDir(projectDir);

      if (!(await fs.pathExists(flowDir))) {
        await prompts.log.warn('No FLOW installation found.');
        process.exit(0);
      }
      ...
      await prompts.intro('FLOW Uninstall');
      ...
      if (!options.yes) {
        ...
        const selected = await prompts.multiselect({
          ...
          options: [
            {
              value: 'modules',
              label: `FLOW Modules & data (${installer.flowFolderName}/)`,
      ...
      // Phase 3: FLOW modules & data (last — other phases may need _flow/)
      if (removeModules) {
        const s = await prompts.spinner();
        s.start(`Removing FLOW modules & data (${installer.flowFolderName}/)...`);
