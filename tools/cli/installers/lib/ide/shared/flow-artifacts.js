...
/**
 * Helpers for gathering FLOW agents/tasks from the installed tree.
 * Shared by installers that need Claude-style exports.
 */
async function getAgentsFromFlow(flowDir, selectedModules = []) {
  const agents = [];

  // Get core agents
  if (await fs.pathExists(path.join(flowDir, 'core', 'agents'))) {
    const coreAgents = await getAgentsFromDir(path.join(flowDir, 'core', 'agents'), 'core');
    agents.push(...coreAgents);
  }

  // Get module agents
  for (const moduleName of selectedModules) {
    const agentsPath = path.join(flowDir, moduleName, 'agents');
...
async function getTasksFromFlow(flowDir, selectedModules = []) {
  const tasks = [];

  if (await fs.pathExists(path.join(flowDir, 'core', 'tasks'))) {
    const coreTasks = await getTasksFromDir(path.join(flowDir, 'core', 'tasks'), 'core');
    tasks.push(...coreTasks);
  }

  for (const moduleName of selectedModules) {
    const tasksPath = path.join(flowDir, moduleName, 'tasks');
