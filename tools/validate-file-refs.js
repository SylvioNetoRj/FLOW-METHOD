 ...
 *
 * Validates cross-file references in FLOW source files (agents, workflows, tasks, steps).
 * Catches broken file paths, missing referenced files, and absolute path leaks.
 *
 * What it checks:
 * - {project-root}/_flow/ references in YAML and markdown resolve to real src/ files
...
// Pattern: {project-root}/_flow/ references
const PROJECT_ROOT_REF = /\{project-root\}\/_flow\/([^\s'"<>})\]`]+)/g;

// Pattern: {_flow}/ shorthand references
const FLOW_SHORTHAND_REF = /\{_flow\}\/([^\s'"<>})\]`]+)/g;
...
function mapInstalledToSource(refPath) {
  // Strip {project-root}/_flow/ or {_flow}/ prefix
  let cleaned = refPath.replace(/^\{project-root\}\/_flow\//, '').replace(/^\{_flow\}\//, '');

  // Also handle bare _flow/ prefix (seen in some invoke-task)
  cleaned = cleaned.replace(/^_flow\//, '');
...
function isInstallOnly(cleanedPath) {
  // Skip paths that only exist in the installed _flow/ structure, not in src/
...
function extractYamlRefs(filePath, content) {
  ...
  function checkValue(value, range, keyPath) {
    ...
    // Check for {project-root}/_flow/ refs
    const prMatch = value.match(/\{project-root\}\/_flow\/[^\s'"<>})\]`]+/);
    if (prMatch) {
      refs.push({ file: filePath, raw: prMatch[0], type: 'project-root', line, key: keyPath });
    }

    // Check for {_flow}/ refs
    const bmMatch = value.match(/\{_flow\}\/[^\s'"<>})\]`]+/);
    if (bmMatch) {
...
function extractMarkdownRefs(filePath, content) {
  ...
  // {project-root}/_flow/ refs
  runPattern(PROJECT_ROOT_REF, 'project-root');

  // {_flow}/ shorthand
  runPattern(FLOW_SHORTHAND_REF, 'project-root');
...
function resolveRef(ref) {
  ...
  if (ref.type === 'exec-attr') {
    ...
    if (execPath.includes('{_flow}')) {
      return mapInstalledToSource(execPath);
    }
    if (execPath.startsWith('_flow/')) {
      return mapInstalledToSource(execPath);
    }
    // Relative exec path
  ...
  if (ref.type === 'invoke-task') {
    ...
    const prMatch = ref.raw.match(/\{project-root\}\/_flow\/([^\s'"<>})\]`]+)/);
    if (prMatch) return mapInstalledToSource(prMatch[0]);

    const bmMatch = ref.raw.match(/\{_flow\}\/([^\s'"<>})\]`]+)/);
    if (bmMatch) return mapInstalledToSource(bmMatch[0]);

    const bareMatch = ref.raw.match(/_flow\/([^\s'"<>})\]`]+)/);
