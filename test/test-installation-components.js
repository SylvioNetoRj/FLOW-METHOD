...
async function createTestFlowFixture() {
  const fixtureDir = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-fixture-'));
  ...
  await fs.writeFile(path.join(fixtureDir, 'core', 'agents', 'flow-master.md'), minimalAgent);
  // Skill manifest so the installer uses 'flow-master' as the canonical skill name
  await fs.writeFile(path.join(fixtureDir, 'core', 'agents', 'flow-skill-manifest.yaml'), 'flow-master.md:\n  canonicalId: flow-master\n');
...
async function runTests() {
  ...
  try {
    ...
    const testPath = '{project-root}/flow/bmm/config.yaml';
    const expectedPattern = /\/flow\/bmm\/config\.yaml$/;
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-windsurf-test-'));
    const installedFlowDir = await createTestFlowFixture();
    const legacyDir = path.join(tempProjectDir, '.windsurf', 'workflows', 'flow-legacy-dir');
    await fs.ensureDir(legacyDir);
    await fs.writeFile(path.join(tempProjectDir, '.windsurf', 'workflows', 'flow-legacy.md'), 'legacy\n');
    await fs.writeFile(path.join(legacyDir, 'SKILL.md'), 'legacy\n');

    const ideManager = new IdeManager();
    await ideManager.ensureInitialized();
    const result = await ideManager.setup('windsurf', tempProjectDir, installedFlowDir, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile = path.join(tempProjectDir, '.windsurf', 'skills', 'flow-master', 'SKILL.md');
    ...
    await fs.remove(installedFlowDir);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-kiro-test-'));
    const installedFlowDir = await createTestFlowFixture();
    const legacyDir = path.join(tempProjectDir, '.kiro', 'steering', 'flow-legacy-dir');
    await fs.ensureDir(legacyDir);
    await fs.writeFile(path.join(tempProjectDir, '.kiro', 'steering', 'flow-legacy.md'), 'legacy\n');
    await fs.writeFile(path.join(legacyDir, 'SKILL.md'), 'legacy\n');

    const ideManager = new IdeManager();
    await ideManager.ensureInitialized();
    const result = await ideManager.setup('kiro', tempProjectDir, installedFlowDir, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile = path.join(tempProjectDir, '.kiro', 'skills', 'flow-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-antigravity-test-'));
    const installedFlowDir = await createTestFlowFixture();
    const legacyDir = path.join(tempProjectDir, '.agent', 'workflows', 'flow-legacy-dir');
    await fs.ensureDir(legacyDir);
    await fs.writeFile(path.join(tempProjectDir, '.agent', 'workflows', 'flow-legacy.md'), 'legacy\n');
    await fs.writeFile(path.join(legacyDir, 'SKILL.md'), 'legacy\n');

    const ideManager = new IdeManager();
    await ideManager.ensureInitialized();
    const result = await ideManager.setup('antigravity', tempProjectDir, installedFlowDir, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile = path.join(tempProjectDir, '.agent', 'skills', 'flow-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-auggie-test-'));
    const installedFlowDir = await createTestFlowFixture();
    const legacyDir = path.join(tempProjectDir, '.augment', 'commands', 'flow-legacy-dir');
    await fs.ensureDir(legacyDir);
    await fs.writeFile(path.join(tempProjectDir, '.augment', 'commands', 'flow-legacy.md'), 'legacy\n');
    await fs.writeFile(path.join(legacyDir, 'SKILL.md'), 'legacy\n');

    const ideManager = new IdeManager();
    await ideManager.ensureInitialized();
    const result = await ideManager.setup('auggie', tempProjectDir, installedFlowDir, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile = path.join(tempProjectDir, '.augment', 'skills', 'flow-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-opencode-test-'));
    const installedFlowDir = await createTestFlowFixture();
    const legacyDirs = [
      path.join(tempProjectDir, '.opencode', 'agents', 'flow-legacy-agent'),
      path.join(tempProjectDir, '.opencode', 'commands', 'flow-legacy-command'),
      path.join(tempProjectDir, '.opencode', 'agent', 'flow-legacy-agent-singular'),
      path.join(tempProjectDir, '.opencode', 'command', 'flow-legacy-command-singular'),
    ];
    ...
    const ideManager = new IdeManager();
    await ideManager.ensureInitialized();
    const result = await ideManager.setup('opencode', tempProjectDir, installedFlowDir, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile = path.join(tempProjectDir, '.opencode', 'skills', 'flow-master', 'SKILL.md');
  ...
  try {
    ...
    const tempProjectDir9 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-claude-code-test-'));
    const installedFlowDir9 = await createTestFlowFixture();
    const legacyDir9 = path.join(tempProjectDir9, '.claude', 'commands');
    await fs.ensureDir(legacyDir9);
    await fs.writeFile(path.join(legacyDir9, 'flow-legacy.md'), 'legacy\n');

    const ideManager9 = new IdeManager();
    await ideManager9.ensureInitialized();
    const result9 = await ideManager9.setup('claude-code', tempProjectDir9, installedFlowDir9, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile9 = path.join(tempProjectDir9, '.claude', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch9 && nameMatch9[1].trim() === 'flow-master', 'Claude Code skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(legacyDir9)), 'Claude Code setup removes legacy commands dir');

    await fs.remove(tempProjectDir9);
    await fs.remove(installedFlowDir9);
  } catch (error) {
  ...
  try {
    const tempRoot10 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-claude-code-ancestor-test-'));
    const parentProjectDir10 = path.join(tempRoot10, 'parent');
    const childProjectDir10 = path.join(parentProjectDir10, 'child');
    const installedFlowDir10 = await createTestFlowFixture();

    await fs.ensureDir(path.join(parentProjectDir10, '.git'));
    await fs.ensureDir(path.join(parentProjectDir10, '.claude', 'skills', 'flow-existing'));
    await fs.ensureDir(childProjectDir10);
    await fs.writeFile(path.join(parentProjectDir10, '.claude', 'skills', 'flow-existing', 'SKILL.md'), 'legacy\n');

    const ideManager10 = new IdeManager();
    await ideManager10.ensureInitialized();
    const result10 = await ideManager10.setup('claude-code', childProjectDir10, installedFlowDir10, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir10);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir11 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-codex-test-'));
    const installedFlowDir11 = await createTestFlowFixture();
    const legacyDir11 = path.join(tempProjectDir11, '.codex', 'prompts');
    await fs.ensureDir(legacyDir11);
    await fs.writeFile(path.join(legacyDir11, 'flow-legacy.md'), 'legacy\n');

    const ideManager11 = new IdeManager();
    await ideManager11.ensureInitialized();
    const result11 = await ideManager11.setup('codex', tempProjectDir11, installedFlowDir11, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile11 = path.join(tempProjectDir11, '.agents', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch11 && nameMatch11[1].trim() === 'flow-master', 'Codex skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(legacyDir11)), 'Codex setup removes legacy prompts dir');

    await fs.remove(tempProjectDir11);
    await fs.remove(installedFlowDir11);
  } catch (error) {
  ...
  try {
    const tempRoot12 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-codex-ancestor-test-'));
    const parentProjectDir12 = path.join(tempRoot12, 'parent');
    const childProjectDir12 = path.join(parentProjectDir12, 'child');
    const installedFlowDir12 = await createTestFlowFixture();

    await fs.ensureDir(path.join(parentProjectDir12, '.git'));
    await fs.ensureDir(path.join(parentProjectDir12, '.agents', 'skills', 'flow-existing'));
    await fs.ensureDir(childProjectDir12);
    await fs.writeFile(path.join(parentProjectDir12, '.agents', 'skills', 'flow-existing', 'SKILL.md'), 'legacy\n');

    const ideManager12 = new IdeManager();
    await ideManager12.ensureInitialized();
    const result12 = await ideManager12.setup('codex', childProjectDir12, installedFlowDir12, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir12);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir13c = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-cursor-test-'));
    const installedFlowDir13c = await createTestFlowFixture();
    const legacyDir13c = path.join(tempProjectDir13c, '.cursor', 'commands');
    await fs.ensureDir(legacyDir13c);
    await fs.writeFile(path.join(legacyDir13c, 'flow-legacy.md'), 'legacy\n');

    const ideManager13c = new IdeManager();
    await ideManager13c.ensureInitialized();
    const result13c = await ideManager13c.setup('cursor', tempProjectDir13c, installedFlowDir13c, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile13c = path.join(tempProjectDir13c, '.cursor', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch13c && nameMatch13c[1].trim() === 'flow-master', 'Cursor skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(legacyDir13c)), 'Cursor setup removes legacy commands dir');

    await fs.remove(tempProjectDir13c);
    await fs.remove(installedFlowDir13c);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir13 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-roo-test-'));
    const installedFlowDir13 = await createTestFlowFixture();
    const legacyDir13 = path.join(tempProjectDir13, '.roo', 'commands', 'flow-legacy-dir');
    await fs.ensureDir(legacyDir13);
    await fs.writeFile(path.join(tempProjectDir13, '.roo', 'commands', 'flow-legacy.md'), 'legacy\n');
    ...
    const result13 = await ideManager13.setup('roo', tempProjectDir13, installedFlowDir13, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile13 = path.join(tempProjectDir13, '.roo', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(
      nameMatch13 && nameMatch13[1].trim() === 'flow-master',
    ...
    const result13b = await ideManager13.setup('roo', tempProjectDir13, installedFlowDir13, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir13);
  } catch (error) {
  ...
  try {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-opencode-ancestor-test-'));
    ...
    await fs.ensureDir(path.join(parentProjectDir, '.opencode', 'skills', 'flow-existing'));
    await fs.ensureDir(childProjectDir);
    await fs.writeFile(path.join(parentProjectDir, '.opencode', 'skills', 'flow-existing', 'SKILL.md'), 'legacy\n');

    const ideManager = new IdeManager();
    await ideManager.ensureInitialized();
    const result = await ideManager.setup('opencode', childProjectDir, installedFlowDir, {
      silent: true,
      selectedModules: ['bmm'],
    });
  ...
  try {
    ...
    const tempProjectDir17 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-copilot-test-'));
    const installedFlowDir17 = await createTestFlowFixture();
    ...
    await fs.writeFile(path.join(legacyAgentsDir17, 'flow-legacy.agent.md'), 'legacy agent\n');
    await fs.writeFile(path.join(legacyPromptsDir17, 'flow-legacy.prompt.md'), 'legacy prompt\n');

    // Create legacy copilot-instructions.md with FLOW markers
    const copilotInstructionsPath17 = path.join(tempProjectDir17, '.github', 'copilot-instructions.md');
    await fs.writeFile(
      copilotInstructionsPath17,
      'User content before\n<!-- FLOW:START -->\nFLOW generated content\n<!-- FLOW:END -->\nUser content after\n',
    );

    const ideManager17 = new IdeManager();
    await ideManager17.ensureInitialized();
    const result17 = await ideManager17.setup('github-copilot', tempProjectDir17, installedFlowDir17, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile17 = path.join(tempProjectDir17, '.github', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch17 && nameMatch17[1].trim() === 'flow-master', 'GitHub Copilot skill name frontmatter matches directory name exactly');
    ...
    // Verify copilot-instructions.md FLOW markers were stripped but user content preserved
    const cleanedInstructions17 = await fs.readFile(copilotInstructionsPath17, 'utf8');
    assert(
      !cleanedInstructions17.includes('FLOW:START') && !cleanedInstructions17.includes('FLOW generated content'),
      'GitHub Copilot setup strips FLOW markers from copilot-instructions.md',
    );
    assert(
    ...
    await fs.remove(installedFlowDir17);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir18 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-cline-test-'));
    const installedFlowDir18 = await createTestFlowFixture();
    const legacyDir18 = path.join(tempProjectDir18, '.clinerules', 'workflows', 'flow-legacy-dir');
    await fs.ensureDir(legacyDir18);
    await fs.writeFile(path.join(tempProjectDir18, '.clinerules', 'workflows', 'flow-legacy.md'), 'legacy\n');
    ...
    const result18 = await ideManager18.setup('cline', tempProjectDir18, installedFlowDir18, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile18 = path.join(tempProjectDir18, '.cline', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch18 && nameMatch18[1].trim() === 'flow-master', 'Cline skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(path.join(tempProjectDir18, '.clinerules', 'workflows'))), 'Cline setup removes legacy workflows dir');

    // Reinstall/upgrade: run setup again over existing skills output
    const result18b = await ideManager18.setup('cline', tempProjectDir18, installedFlowDir18, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir18);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir19 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-codebuddy-test-'));
    const installedFlowDir19 = await createTestFlowFixture();
    const legacyDir19 = path.join(tempProjectDir19, '.codebuddy', 'commands', 'flow-legacy-dir');
    await fs.ensureDir(legacyDir19);
    await fs.writeFile(path.join(tempProjectDir19, '.codebuddy', 'commands', 'flow-legacy.md'), 'legacy\n');
    ...
    const result19 = await ideManager19.setup('codebuddy', tempProjectDir19, installedFlowDir19, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile19 = path.join(tempProjectDir19, '.codebuddy', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch19 && nameMatch19[1].trim() === 'flow-master', 'CodeBuddy skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(path.join(tempProjectDir19, '.codebuddy', 'commands'))), 'CodeBuddy setup removes legacy commands dir');

    const result19b = await ideManager19.setup('codebuddy', tempProjectDir19, installedFlowDir19, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir19);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir20 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-crush-test-'));
    const installedFlowDir20 = await createTestFlowFixture();
    const legacyDir20 = path.join(tempProjectDir20, '.crush', 'commands', 'flow-legacy-dir');
    await fs.ensureDir(legacyDir20);
    await fs.writeFile(path.join(tempProjectDir20, '.crush', 'commands', 'flow-legacy.md'), 'legacy\n');
    ...
    const result20 = await ideManager20.setup('crush', tempProjectDir20, installedFlowDir20, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile20 = path.join(tempProjectDir20, '.crush', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch20 && nameMatch20[1].trim() === 'flow-master', 'Crush skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(path.join(tempProjectDir20, '.crush', 'commands'))), 'Crush setup removes legacy commands dir');

    const result20b = await ideManager20.setup('crush', tempProjectDir20, installedFlowDir20, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir20);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir21 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-trae-test-'));
    const installedFlowDir21 = await createTestFlowFixture();
    const legacyDir21 = path.join(tempProjectDir21, '.trae', 'rules');
    await fs.ensureDir(legacyDir21);
    await fs.writeFile(path.join(legacyDir21, 'flow-legacy.md'), 'legacy\n');

    const ideManager21 = new IdeManager();
    await ideManager21.ensureInitialized();
    const result21 = await ideManager21.setup('trae', tempProjectDir21, installedFlowDir21, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile21 = path.join(tempProjectDir21, '.trae', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch21 && nameMatch21[1].trim() === 'flow-master', 'Trae skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(path.join(tempProjectDir21, '.trae', 'rules'))), 'Trae setup removes legacy rules dir');

    const result21b = await ideManager21.setup('trae', tempProjectDir21, installedFlowDir21, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir21);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir22 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-kilo-test-'));
    const installedFlowDir22 = await createTestFlowFixture();
    ...
    await fs.writeFile(path.join(legacyDir22, 'flow-legacy.md'), 'legacy\n');

    const result22 = await ideManager22.setup('kilo', tempProjectDir22, installedFlowDir22, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir22);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir23 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-gemini-test-'));
    const installedFlowDir23 = await createTestFlowFixture();
    const legacyDir23 = path.join(tempProjectDir23, '.gemini', 'commands');
    await fs.ensureDir(legacyDir23);
    await fs.writeFile(path.join(legacyDir23, 'flow-legacy.toml'), 'legacy\n');

    const ideManager23 = new IdeManager();
    await ideManager23.ensureInitialized();
    const result23 = await ideManager23.setup('gemini', tempProjectDir23, installedFlowDir23, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile23 = path.join(tempProjectDir23, '.gemini', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch23 && nameMatch23[1].trim() === 'flow-master', 'Gemini skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(path.join(tempProjectDir23, '.gemini', 'commands'))), 'Gemini setup removes legacy commands dir');

    const result23b = await ideManager23.setup('gemini', tempProjectDir23, installedFlowDir23, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    await fs.remove(installedFlowDir23);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir24 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-iflow-test-'));
    const installedFlowDir24 = await createTestFlowFixture();
    const legacyDir24 = path.join(tempProjectDir24, '.iflow', 'commands');
    await fs.ensureDir(legacyDir24);
    await fs.writeFile(path.join(legacyDir24, 'flow-legacy.md'), 'legacy\n');

    const ideManager24 = new IdeManager();
    await ideManager24.ensureInitialized();
    const result24 = await ideManager24.setup('iflow', tempProjectDir24, installedFlowDir24, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile24 = path.join(tempProjectDir24, '.iflow', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch24 && nameMatch24[1].trim() === 'flow-master', 'iFlow skill name frontmatter matches directory name exactly');
    ...
    await fs.remove(installedFlowDir24);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir25 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-qwen-test-'));
    const installedFlowDir25 = await createTestFlowFixture();
    const legacyDir25 = path.join(tempProjectDir25, '.qwen', 'commands');
    await fs.ensureDir(legacyDir25);
    await fs.writeFile(path.join(legacyDir25, 'flow-legacy.md'), 'legacy\n');

    const ideManager25 = new IdeManager();
    await ideManager25.ensureInitialized();
    const result25 = await ideManager25.setup('qwen', tempProjectDir25, installedFlowDir25, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile25 = path.join(tempProjectDir25, '.qwen', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch25 && nameMatch25[1].trim() === 'flow-master', 'QwenCoder skill name frontmatter matches directory name exactly');
    ...
    await fs.remove(installedFlowDir25);
  } catch (error) {
  ...
  try {
    ...
    const tempProjectDir26 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-rovodev-test-'));
    const installedFlowDir26 = await createTestFlowFixture();
    const legacyDir26 = path.join(tempProjectDir26, '.rovodev', 'workflows');
    await fs.ensureDir(legacyDir26);
    await fs.writeFile(path.join(legacyDir26, 'flow-legacy.md'), 'legacy\n');

    // Create a prompts.yml with FLOW entries and a user entry
    ...
    const promptsContent26 = yaml26.stringify({
      prompts: [
        { name: 'flow-bmm-create-prd', description: 'FLOW workflow', content_file: 'workflows/flow-bmm-create-prd.md' },
    ...
    const result26 = await ideManager26.setup('rovo-dev', tempProjectDir26, installedFlowDir26, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile26 = path.join(tempProjectDir26, '.rovodev', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(nameMatch26 && nameMatch26[1].trim() === 'flow-master', 'Rovo Dev skill name frontmatter matches directory name exactly');

    assert(!(await fs.pathExists(path.join(tempProjectDir26, '.rovodev', 'workflows'))), 'Rovo Dev setup removes legacy workflows dir');

    // Verify prompts.yml cleanup: FLOW entries removed, user entry preserved
    ...
    assert(
      ...
      'Rovo Dev cleanup removes FLOW entries from prompts.yml',
    );
    assert(cleanedPrompts26.prompts[0].name === 'my-custom-prompt', 'Rovo Dev cleanup preserves non-FLOW entries in prompts.yml');

    await fs.remove(tempProjectDir26);
    await fs.remove(installedFlowDir26);
  } catch (error) {
  ...
  // ============================================================
  // Suite 27: Cleanup preserves flow-os-* skills
  // ============================================================
  console.log(`${colors.yellow}Test Suite 27: Cleanup preserves flow-os-* skills${colors.reset}\n`);

  try {
    const tempProjectDir27 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-os-preserve-test-'));
    const installedFlowDir27 = await createTestFlowFixture();

    // Pre-populate .claude/skills with flow-os-* skills (version-controlled repo skills)
    const osSkillDir27 = path.join(tempProjectDir27, '.claude', 'skills', 'flow-os-review-pr');
    await fs.ensureDir(osSkillDir27);
    await fs.writeFile(
      path.join(osSkillDir27, 'SKILL.md'),
      '---\nname: flow-os-review-pr\ndescription: Review PRs\n---\nOS skill content\n',
    );

    const osSkillDir27b = path.join(tempProjectDir27, '.claude', 'skills', 'flow-os-release-module');
    await fs.ensureDir(osSkillDir27b);
    await fs.writeFile(
      path.join(osSkillDir27b, 'SKILL.md'),
      '---\nname: flow-os-release-module\ndescription: Release module\n---\nOS skill content\n',
    );

    // Also add a regular flow skill that SHOULD be cleaned up
    const regularSkillDir27 = path.join(tempProjectDir27, '.claude', 'skills', 'flow-architect');
    ...
    const result27 = await ideManager27.setup('claude-code', tempProjectDir27, installedFlowDir27, {
      silent: true,
      selectedModules: ['bmm'],
    });

    assert(result27.success === true, 'Claude Code setup succeeds with flow-os-* skills present');

    // flow-os-* skills must survive
    assert(await fs.pathExists(osSkillDir27), 'Cleanup preserves flow-os-review-pr skill');
    assert(await fs.pathExists(osSkillDir27b), 'Cleanup preserves flow-os-release-module skill');

    // flow-os skill content must be untouched
    const osContent27 = await fs.readFile(path.join(osSkillDir27, 'SKILL.md'), 'utf8');
    assert(osContent27.includes('OS skill content'), 'flow-os-review-pr skill content is unchanged');

    // Regular flow skill should have been replaced by fresh install
    const newSkillFile27 = path.join(tempProjectDir27, '.claude', 'skills', 'flow-master', 'SKILL.md');
    assert(await fs.pathExists(newSkillFile27), 'Fresh flow skills are installed alongside preserved flow-os-* skills');

    // Stale non-flow-os skill must have been removed by cleanup
    assert(!(await fs.pathExists(regularSkillDir27)), 'Cleanup removes stale non-flow-os skills');

    await fs.remove(tempProjectDir27);
    await fs.remove(installedFlowDir27);
  } catch (error) {
    assert(false, 'flow-os-* skill preservation test succeeds', error.message);
  }
  ...
  try {
    ...
    tempProjectDir28 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-pi-test-'));
    installedFlowDir28 = await createTestFlowFixture();
    ...
    const result28 = await ideManager28.setup('pi', tempProjectDir28, installedFlowDir28, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    const skillFile28 = path.join(tempProjectDir28, '.pi', 'skills', 'flow-master', 'SKILL.md');
    ...
    assert(fmName28 && fmName28[1].trim() === 'flow-master', 'Pi skill name frontmatter matches directory name exactly');
    ...
    const result28b = await ideManager28.setup('pi', tempProjectDir28, installedFlowDir28, {
      silent: true,
      selectedModules: ['bmm'],
    });
    ...
    if (installedFlowDir28) await fs.remove(installedFlowDir28).catch(() => {});
  }
  ...
  let tempFixture29;
  try {
    tempFixture29 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-skill-scanner-'));
    ...
    await fs.writeFile(path.join(skillDir29, 'flow-skill-manifest.yaml'), 'type: skill\n');
    await fs.writeFile(
    ...
    await fs.writeFile(path.join(wfDir29, 'flow-skill-manifest.yaml'), 'type: workflow\ncanonicalId: regular-wf\n');
    await fs.writeFile(
    ...
    await fs.writeFile(path.join(wfSkillDir29, 'flow-skill-manifest.yaml'), 'type: skill\n');
    await fs.writeFile(
    ...
    await fs.writeFile(path.join(taskSkillDir29, 'flow-skill-manifest.yaml'), 'type: skill\n');
    await fs.writeFile(
    ...
    await fs.writeFile(path.join(skillOnlyModDir29, 'deep', 'nested', 'my-skill', 'flow-skill-manifest.yaml'), 'type: skill\n');
    await fs.writeFile(
  ...
  let tempFixture30;
  try {
    tempFixture30 = await fs.mkdtemp(path.join(os.tmpdir(), 'flow-test-30-'));

    const generator30 = new ManifestGenerator();
    generator30.flowFolderName = '_flow';
