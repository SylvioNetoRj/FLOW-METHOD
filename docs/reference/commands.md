---
title: Skills
description: Reference for FLOW skills — what they are, how they work, and where to find them.
sidebar:
  order: 3
---

Skills are pre-built prompts that load agents, run workflows, or execute tasks inside your IDE. The FLOW installer generates them from your installed modules at install time. If you later add, remove, or change modules, re-run the installer to keep skills in sync (see [Troubleshooting](#troubleshooting)).

## Skills vs. Agent Menu Triggers

FLOW offers two ways to start work, and they serve different purposes.

| Mechanism | How you invoke it | What happens |
| --- | --- | --- |
| **Skill** | Type the skill name (e.g. `flow-help`) in your IDE | Directly loads an agent, runs a workflow, or executes a task |
...
The directory name determines the skill name in your IDE. For example, the directory `flow-dev/` registers the skill `flow-dev`.
...
Run `flow-help` for context-aware guidance on your next step.
...
| --- | --- | --- |
| `flow-dev` | Amelia (Developer) | Implements stories with strict adherence to specs |
| `flow-pm` | John (Product Manager) | Creates and validates PRDs |
| `flow-architect` | Winston (Architect) | Designs system architecture |
| `flow-sm` | Bob (Scrum Master) | Manages sprints and stories |
...
| --- | --- |
| `flow-create-prd` | Create a Product Requirements Document |
| `flow-create-architecture` | Design system architecture |
| `flow-create-epics-and-stories` | Create epics and stories |
| `flow-dev-story` | Implement a story |
| `flow-code-review` | Run a code review |
| `flow-quick-spec` | Define an ad-hoc change (Quick Flow) |
...
#### FLOW-Help: Your Intelligent Guide

**`flow-help`** is your primary interface for discovering what to do next. It's not just a lookup tool — it's an intelligent assistant that:
...
| Example skill | Purpose |
| --- | --- |
| `flow-shard-doc` | Split a large markdown file into smaller sections |
| `flow-index-docs` | Index project documentation |
| `flow-editorial-review-prose` | Review document prose quality |

## Naming Convention

All skills use the `flow-` prefix followed by a descriptive name (e.g., `flow-dev`, `flow-create-prd`, `flow-help`). See [Modules](./modules.md) for available modules.
