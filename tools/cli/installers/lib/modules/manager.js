...
/**
 * Manages the installation, updating, and removal of FLOW modules.
 ...
 * await manager.install('core-module', '/path/to/flow');
 */
class ModuleManager {
  constructor(options = {}) {
    this.xmlHandler = new XmlHandler();
    this.flowFolderName = FLOW_FOLDER_NAME; // Default, can be overridden
    this.customModulePaths = new Map(); // Initialize custom module paths
    this.externalModuleManager = new ExternalModuleManager(); // For external official modules
  }

  /**
   * Set the flow folder name for placeholder replacement
   * @param {string} flowFolderName - The flow folder name
   */
  setFlowFolderName(flowFolderName) {
  ...
  /**
   * Copy sidecar directory to _flow/_memory location with update-safe handling
   * @param {string} sourceSidecarPath - Source sidecar directory path
   * @param {string} agentName - Name of the agent (for naming)
   * @param {string} flowMemoryPath - This should ALWAYS be _flow/_memory
   ...
   */
  async copySidecarToMemory(sourceSidecarPath, agentName, flowMemoryPath, isUpdate = false, flowDir = null, installer = null) {
    const crypto = require('node:crypto');
    const sidecarTargetDir = path.join(flowMemoryPath, `${agentName}-sidecar`);

    // Ensure target directory exists
    await fs.ensureDir(flowMemoryPath);
    ...
    if (isUpdate && installer) {
      existingFilesManifest = await installer.readFilesManifest(flowDir);
    }
    ...
    for (const file of sourceFiles) {
      ...
      // Path relative to flow directory
      const relativeToFlow = path.join('_memory', `${agentName}-sidecar`, file);
      ...
      if (isUpdate && (await fs.pathExists(targetFilePath))) {
        ...
        const lastKnownHash = existingSidecarFiles.get(relativeToFlow);
        ...
        if (lastKnownHash) {
          ...
          if (currentTargetHash === lastKnownHash) {
            ...
            if (process.env.FLOW_VERBOSE_INSTALL === 'true') {
              await prompts.log.message(`    Updated sidecar file: ${relativeToFlow}`);
            }
          } else {
            // User has modified the file, preserve it
            if (process.env.FLOW_VERBOSE_INSTALL === 'true') {
              await prompts.log.message(`    Preserving user-modified file: ${relativeToFlow}`);
            }
          }
        } else {
          ...
          await this.copyFileWithPlaceholderReplacement(sourceFilePath, targetFilePath, true);
          if (process.env.FLOW_VERBOSE_INSTALL === 'true') {
            await prompts.log.message(`    Added new sidecar file: ${relativeToFlow}`);
          }
        }
      } else {
        // New installation
        await this.copyFileWithPlaceholderReplacement(sourceFilePath, targetFilePath, true);
        if (process.env.FLOW_VERBOSE_INSTALL === 'true') {
          await prompts.log.message(`    Copied sidecar file: ${relativeToFlow}`);
        }
      }
  ...
  async listAvailable() {
    ...
    if (this.flowDir) {
      const customCacheDir = path.join(this.flowDir, '_config', 'custom');
  ...
  getExternalCacheDir() {
    ...
    const cacheDir = path.join(os.homedir(), '.flow', 'cache', 'external-modules');
    return cacheDir;
  }
   ...
   * @param {string} flowDir - Target flow directory
   ...
   */
  async install(moduleName, flowDir, fileTrackingCallback = null, options = {}) {
    const sourcePath = await this.findModuleSource(moduleName, { silent: options.silent });
    const targetPath = path.join(flowDir, moduleName);
    ...
    await this.compileModuleAgents(sourcePath, targetPath, moduleName, flowDir, options.installer);
    ...
    if (!options.skipModuleInstaller) {
      await this.createModuleDirectories(moduleName, flowDir, options);
    }

    // Capture version info for manifest
    const { Manifest } = require('../core/manifest');
    const manifestObj = new Manifest();
    const versionInfo = await manifestObj.getModuleVersionInfo(moduleName, flowDir, sourcePath);

    await manifestObj.addModule(flowDir, moduleName, {
   ...
   */
  async update(moduleName, flowDir, force = false, options = {}) {
    ...
    if (force) {
      ...
      await fs.remove(targetPath);
      return await this.install(moduleName, flowDir, null, { installer: options.installer });
    } else {
   ...
   * @param {string} flowDir - Target flow directory
   */
  async remove(moduleName, flowDir) {
    const targetPath = path.join(flowDir, moduleName);
   ...
   */
  async isInstalled(moduleName, flowDir) {
    const targetPath = path.join(flowDir, moduleName);
   ...
   */
  async getInstalledInfo(moduleName, flowDir) {
    const targetPath = path.join(flowDir, moduleName);
   ...
   * @param {Object} installer - Installer instance for file tracking
   */
  async compileModuleAgents(sourcePath, targetPath, moduleName, flowDir, installer = null) {
      ...
      try {
        ...
        if (!(await fs.pathExists(customizePath))) {
          ...
          if (await fs.pathExists(genericTemplatePath)) {
            ...
            manifestData.agentCustomizations[path.relative(flowDir, customizePath)] = originalHash;
        ...
        if (hasSidecar) {
          ...
          if (await fs.pathExists(sourceSidecarPath)) {
            ...
            const flowMemoryPath = path.join(flowDir, '_memory');
            ...
            const copiedFiles = await this.copySidecarToMemory(sourceSidecarPath, agentName, flowMemoryPath, isUpdate, flowDir, installer);

            if (process.env.FLOW_VERBOSE_INSTALL === 'true' && copiedFiles.length > 0) {
              await prompts.log.message(`    Sidecar files processed: ${copiedFiles.length} files`);
            }
          } else if (process.env.FLOW_VERBOSE_INSTALL === 'true') {
  ...
  async vendorCrossModuleWorkflows(sourcePath, targetPath, moduleName) {
    ...
    for (const agentFile of yamlFiles) {
      ...
      for (const item of workflowInstallItems) {
        ...
        // Example: {project-root}/_flow/bmm/workflows/4-implementation/create-story/workflow.md
        const sourceMatch = sourceWorkflowPath.match(/\{project-root\}\/(?:_flow)\/([^/]+)\/workflows\/(.+)/);
        if (!sourceMatch) {
        ...
        // Example: {project-root}/_flow/bmgd/workflows/4-production/create-story/workflow.md
        const installMatch = installWorkflowPath.match(/\{project-root\}\/(?:_flow)\/([^/]+)\/workflows\/(.+)/);
        if (!installMatch) {
   ...
   */
  async createModuleDirectories(moduleName, flowDir, options = {}) {
  ...
  async processModuleConfig(modulePath, moduleName) {
    ...
    if (await fs.pathExists(configPath)) {
      try {
        ...
        configContent = configContent.replaceAll('{project-root}', `flow/${moduleName}`);
