...
description: How to use FLOW Method on existing codebases
sidebar:
  order: 6
---

Use FLOW Method effectively when working on existing projects and legacy codebases.
...
If you have completed all PRD epics and stories through the FLOW process, clean up those files. Archive them, delete them, or rely on version history if needed. Do not keep these files in:

- `docs/`
- `_flow-output/planning-artifacts/`
- `_flow-output/implementation-artifacts/`
...
You can review and refine the generated file, or create it manually at `_flow-output/project-context.md` if you prefer.
...
For complex projects, consider using the `flow-document-project` workflow. It offers runtime variants that will scan your entire project and document its actual current state.

## Step 3: Get Help

### FLOW-Help: Your Starting Point

**Run `flow-help` anytime you're unsure what to do next.** This intelligent guide:
...
```
flow-help I have an existing Rails app, where should I start?
flow-help What's the difference between quick-flow and full method?
flow-help Show me what workflows are available
```

FLOW-Help also **automatically runs at the end of every workflow**, providing clear guidance on exactly what to do next.
...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| **Small updates or additions** | Use `flow-quick-flow-solo-dev` to create a tech-spec and implement the change. The full four-phase FLOW Method is likely overkill. |
| **Major changes or additions** | Start with the FLOW Method, applying as much or as little rigor as needed.                                                    |
