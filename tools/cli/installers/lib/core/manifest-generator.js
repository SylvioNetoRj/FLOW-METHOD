...
const {
  ...
  getInstallToFlow: getInstallToFlowShared,
...
class ManifestGenerator {
  ...
  /** Delegate to shared skill-manifest module */
  getInstallToFlow(manifest, filename) {
    return getInstallToFlowShared(manifest, filename);
  }
   ...
   */
  async generateManifests(flowDir, selectedModules, installedFiles = [], options = {}) {
    ...
    // Scan the flow directory to find all actually installed modules
    const installedModules = await this.scanInstalledModules(flowDir);
    ...
    this.flowFolderName = path.basename(flowDir); // Get the actual folder name (e.g., '_flow' or 'flow')
   ...
   * A skill directory is one that contains both a flow-skill-manifest.yaml with
  ...
  async collectSkills() {
    ...
    const debug = process.env.FLOW_DEBUG_MANIFEST === 'true';

    for (const moduleName of this.updatedModules) {
      const modulePath = path.join(this.flowDir, moduleName);
        ...
        if (artifactType === 'skill') {
          ...
          if (skillMeta) {
            ...
            const installPath = relativePath
              ? `${this.flowFolderName}/${moduleName}/${relativePath}/${skillFile}`
              : `${this.flowFolderName}/${moduleName}/${skillFile}`;
            ...
            if (manifest && manifest.__single && manifest.__single.canonicalId) {
              console.warn(
                `Warning: Skill manifest at ${dir}/flow-skill-manifest.yaml contains canonicalId — this field is ignored for skills (directory name is the canonical ID)`,
              );
            }
            ...
            this.skills.push({
              ...
              path: installPath,
              canonicalId,
              install_to_flow: this.getInstallToFlow(manifest, skillFile),
            });
   ...
   * Scans the INSTALLED flow directory, not the source
   */
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (workflow.name && workflow.description) {
              ...
              const installPath =
                moduleName === 'core'
                  ? `${this.flowFolderName}/core/${subDir}/${relativePath}/${entry.name}`
                  : `${this.flowFolderName}/${moduleName}/${subDir}/${relativePath}/${entry.name}`;
  ...
  async collectAgents(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const agentsPath = path.join(this.flowDir, moduleName, 'agents');
    ...
    // Get standalone agents from flow/agents/ directory
    const standaloneAgentsDir = path.join(this.flowDir, 'agents');
  ...
  async getAgentsFromDir(dirPath, moduleName, relativePath = '') {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const installPath =
          moduleName === 'core'
            ? `${this.flowFolderName}/core/agents/${fileRelativePath}`
            : `${this.flowFolderName}/${moduleName}/agents/${fileRelativePath}`;
  ...
  async collectTasks(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const tasksPath = path.join(this.flowDir, moduleName, 'tasks');
  ...
  async getTasksFromDir(dirPath, moduleName) {
    ...
    for (const file of files) {
      ...
      if (file.endsWith('.xml') || file.endsWith('.md')) {
        ...
        // Build relative path for installation
        const installPath =
          moduleName === 'core' ? `${this.flowFolderName}/core/tasks/${file}` : `${this.flowFolderName}/${moduleName}/tasks/${file}`;
  ...
  async collectTools(selectedModules) {
    ...
    // Use updatedModules which already includes deduplicated 'core' + selectedModules
    for (const moduleName of this.updatedModules) {
      const toolsPath = path.join(this.flowDir, moduleName, 'tools');
  ...
  async getToolsFromDir(dirPath, moduleName) {
    ...
    for (const file of files) {
      ...
      if (file.endsWith('.xml') || file.endsWith('.md')) {
        ...
        // Build relative path for installation
        const installPath =
          moduleName === 'core' ? `${this.flowFolderName}/core/tools/${file}` : `${this.flowFolderName}/${moduleName}/tools/${file}`;
  ...
  async writeMainManifest(cfgDir) {
    ...
    for (const moduleName of this.modules) {
      ...
      const versionInfo = await manifestObj.getModuleVersionInfo(moduleName, this.flowDir);
  ...
  async writeSkillManifest(cfgDir) {
    ...
    let csvContent = 'canonicalId,name,description,module,path,install_to_flow\n';
    ...
    for (const skill of this.skills) {
      const row = [
        ...
        escapeCsv(skill.install_to_flow),
      ].join(',');
      csvContent += row + '\n';
    }
  ...
  async writeFilesManifest(cfgDir) {
    ...
    if (this.allInstalledFiles && this.allInstalledFiles.length > 0) {
      ...
      for (const filePath of this.allInstalledFiles) {
        // Store paths relative to flowDir (no folder prefix)
        const relativePath = filePath.replace(this.flowDir, '').replaceAll('\\', '/').replace(/^\//, '');
      ...
      for (const file of this.files) {
        ...
        const relPath = file.path.replace(this.flowFolderName + '/', '');
        const filePath = path.join(this.flowDir, relPath);
        const hash = await this.calculateFileHash(filePath);
        allFiles.push({
          ...file,
          path: relPath,
  ...
  /**
   * Scan the flow directory to find all installed modules
   * @param {string} flowDir - Path to flow directory
   * @returns {Array} List of module names
   */
  async scanInstalledModules(flowDir) {
    const modules = [];
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const modulePath = path.join(flowDir, entry.name);
        ...
        // Check for skill-only modules: recursive scan for flow-skill-manifest.yaml with type: skill
        let hasSkills = false;
  ...
  /**
   * Recursively check if a directory tree contains a flow-skill-manifest.yaml with type: skill.
