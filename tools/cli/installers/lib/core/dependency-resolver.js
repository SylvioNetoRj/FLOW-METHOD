...
/**
 * Dependency Resolver for FLOW modules
...
class DependencyResolver {
   ...
   */
  async resolve(flowDir, selectedModules = [], options = {}) {
    if (options.verbose) {
    ...
    const primaryFiles = await this.collectPrimaryFiles(flowDir, modulesToProcess, options);
    ...
    const resolvedDeps = await this.resolveDependencyPaths(flowDir, allDependencies);

    // Fourth pass: check for transitive dependencies
    const transitiveDeps = await this.resolveTransitiveDependencies(flowDir, resolvedDeps);

    // Combine all files
    const allFiles = new Set([...primaryFiles.map((f) => f.path), ...resolvedDeps, ...transitiveDeps]);

    // Organize by module
    const organizedFiles = this.organizeByModule(flowDir, allFiles);
   ...
   */
  async collectPrimaryFiles(flowDir, modules, options = {}) {
    const files = [];
    ...
    for (const module of modules) {
      ...
      // Handle both source (src/) and installed (flow/) directory structures
      let moduleDir;
  ...
  async parseDependencies(files) {
    ...
    for (const file of files) {
      ...
      for (const ref of fileRefs) {
        ...
        // Paths starting with flow/ are absolute references to the flow installation
        const depType = ref.startsWith('flow/') ? 'flow-path' : 'file';
        allDeps.add({
          from: file.path,
          dependency: ref,
          type: depType,
  ...
  parseCommandReferences(content) {
    ...
    const commandPattern = /@(task-|agent-|flow-)([a-z0-9-]+)/g;
    let match;
    ...
    // Match file paths like flow/core/agents/analyst
    const pathPattern = /flow\/(core|bmm|cis)\/(agents|tasks)\/([a-z0-9-]+)/g;
  ...
  parseFileReferences(content) {
    ...
    while ((match = execPattern.exec(content)) !== null) {
      ...
      if (execPath && execPath !== '*') {
        ...
        // Usage is like {project-root}/flow/core/tasks/foo.md
   ...
   */
  async resolveDependencyPaths(flowDir, dependencies) {
    const resolved = new Set();

    for (const dep of dependencies) {
      const resolvedPaths = await this.resolveSingleDependency(flowDir, dep);
   ...
   */
  async resolveSingleDependency(flowDir, dep) {
    const paths = [];
    ...
    switch (dep.type) {
      ...
      case 'file': {
        ...
        if (depPath.includes('{project-root}')) {
          // Remove {project-root} and resolve as flow path
          depPath = depPath.replace('{project-root}', '');

          if (depPath.startsWith('flow/')) {
            const flowPath = depPath.replace(/^flow\//, '');

            // Handle glob patterns
            if (depPath.includes('*')) {
              // Extract the base path and pattern
              const pathParts = flowPath.split('/');
              ...
              let basePath;
              if (module === 'core') {
                basePath = path.join(flowDir, 'core', middlePath);
              } else {
                basePath = path.join(flowDir, 'modules', module, middlePath);
              }
              ...
              // Direct path
              if (flowPath.startsWith('core/')) {
                const corePath = path.join(flowDir, flowPath);
                ...
                const modulePath = path.join(flowDir, 'modules', module, rest);
      ...
      case 'command': {
        ...
        const commandPath = await this.resolveCommandToPath(flowDir, dep.dependency);
        if (commandPath) {
      ...
      case 'flow-path': {
        // Resolve flow/ paths (from {project-root}/flow/... references)
        // These are paths relative to the src directory structure
        const flowPath = dep.dependency.replace(/^flow\//, '');

        // Try to resolve as if it's in src structure
        // flow/core/tasks/foo.md -> src/core/tasks/foo.md
        // flow/bmm/tasks/bar.md -> src/bmm/tasks/bar.md (bmm is directly under src/)
        // flow/cis/agents/bar.md -> src/modules/cis/agents/bar.md
        ...
        if (flowPath.startsWith('core/')) {
          ...
          if (module === 'bmm') {
            ...
            modulePath = path.join(flowDir, module, rest);
          } else {
            // Other modules are under modules/
            modulePath = path.join(flowDir, 'modules', module, rest);
          }
      ...
      case 'template': {
        ...
        if (templateDep.includes('{project-root}')) {
          // Remove {project-root} and treat as flow-path
          templateDep = templateDep.replace('{project-root}', '');

          // Now resolve as a flow path
          if (templateDep.startsWith('flow/')) {
            const flowPath = templateDep.replace(/^flow\//, '');
   ...
   */
  async resolveCommandToPath(flowDir, command) {
    // Parse command format: @task-name or @agent-name or flow/module/type/name
    ...
    if (command.startsWith('@task-')) {
      ...
      for (const module of ['core', 'bmm', 'cis']) {
        const taskPath =
          module === 'core'
            ? path.join(flowDir, 'core', 'tasks', `${taskName}.md`)
            : path.join(flowDir, 'modules', module, 'tasks', `${taskName}.md`);
      ...
      for (const module of ['core', 'bmm', 'cis']) {
        const agentPath =
          module === 'core'
            ? path.join(flowDir, 'core', 'agents', `${agentName}.md`)
            : path.join(flowDir, 'modules', module, 'agents', `${agentName}.md`);
      ...
      if (parts.length >= 4) {
        ...
        const filePath =
          module === 'core' ? path.join(flowDir, 'core', type, fileName) : path.join(flowDir, 'modules', module, type, fileName);
   ...
   */
  async resolveTransitiveDependencies(flowDir, directDeps) {
        ...
        const subDeps = await this.parseDependencies([
          {
            path: depPath,
            type: 'dependency',
            module: this.getModuleFromPath(flowDir, depPath),
            name: path.basename(depPath),
          },
        ]);

        const resolvedSubDeps = await this.resolveDependencyPaths(flowDir, subDeps);
   ...
   */
  getModuleFromPath(flowDir, filePath) {
   ...
   */
  organizeByModule(flowDir, files) {
    const organized = {};

    for (const file of files) {
      const module = this.getModuleFromPath(flowDir, file);
      if (!organized[module]) {
        organized[module] = {
          agents: [],
          tasks: [],
      ...
      if (file.includes('/src/core/') || file.includes('/src/bmm/')) {
        if (module === 'core') {
          moduleBase = path.join(flowDir, 'src', 'core');
        } else if (module === 'bmm') {
          moduleBase = path.join(flowDir, 'src', 'bmm');
        }
      } else {
        moduleBase = module === 'core' ? path.join(flowDir, 'core') : path.join(flowDir, 'modules', module);
      }
