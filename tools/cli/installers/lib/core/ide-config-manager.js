 ...
 * Saves and loads IDE-specific configurations to/from flow/_config/ides/
 */
class IdeConfigManager {
   ...
   */
  getIdeConfigDir(flowDir) {
    return path.join(flowDir, '_config', 'ides');
  }
   ...
   */
  getIdeConfigPath(flowDir, ideName) {
    return path.join(this.getIdeConfigDir(flowDir), `${ideName}.yaml`);
  }
   ...
   */
  async saveIdeConfig(flowDir, ideName, configuration) {
    const configDir = this.getIdeConfigDir(flowDir);
    await fs.ensureDir(configDir);

    const configPath = this.getIdeConfigPath(flowDir, ideName);
    ...
    if (await fs.pathExists(configPath)) {
      try {
        const existing = await this.loadIdeConfig(flowDir, ideName);
   ...
   */
  async loadIdeConfig(flowDir, ideName) {
    const configPath = this.getIdeConfigPath(flowDir, ideName);
   ...
   */
  async loadAllIdeConfigs(flowDir) {
    const configDir = this.getIdeConfigDir(flowDir);
    const configs = {};
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.yaml')) {
          ...
          const config = await this.loadIdeConfig(flowDir, ideName);
          if (config) {
   ...
   */
  async hasIdeConfig(flowDir, ideName) {
    const configPath = this.getIdeConfigPath(flowDir, ideName);
   ...
   * @param {string} ideName - IDE name
   */
  async deleteIdeConfig(flowDir, ideName) {
    const configPath = this.getIdeConfigPath(flowDir, ideName);
    if (await fs.pathExists(configPath)) {
