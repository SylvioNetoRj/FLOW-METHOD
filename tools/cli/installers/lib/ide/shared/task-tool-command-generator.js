...
const { toColonName, toColonPath, toDashPath, FLOW_FOLDER_NAME } = require('./path-utils');
...
class TaskToolCommandGenerator {
  /**
   * @param {string} flowFolderName - Name of the FLOW folder for template rendering (default: '_flow')
   ...
   * filesystem paths with '_flow/' prefix (the actual folder name), while flowFolderName is
   * used for template placeholder rendering ({{flowFolderName}}).
   */
  constructor(flowFolderName = FLOW_FOLDER_NAME) {
   ...
   */
  async collectTaskToolArtifacts(flowDir) {
    const tasks = await this.loadTaskManifest(flowDir);
    const tools = await this.loadToolManifest(flowDir);

    // All tasks/tools in manifest are standalone (internal=true items are filtered during manifest generation)
    const artifacts = [];
    const flowPrefix = `${FLOW_FOLDER_NAME}/`;
    ...
    for (const task of tasks || []) {
      ...
      if (path.isAbsolute(taskPath)) {
        taskPath = path.relative(flowDir, taskPath).replaceAll('\\', '/');
      }
      // Remove _flow/ prefix if present to get relative path within flow folder
      if (taskPath.startsWith(flowPrefix)) {
        taskPath = taskPath.slice(flowPrefix.length);
      }
    ...
    for (const tool of tools || []) {
      ...
      if (path.isAbsolute(toolPath)) {
        toolPath = path.relative(flowDir, toolPath).replaceAll('\\', '/');
      }
      // Remove _flow/ prefix if present to get relative path within flow folder
      if (toolPath.startsWith(flowPrefix)) {
        toolPath = toolPath.slice(flowPrefix.length);
      }
   ...
   * @param {string} flowDir - FLOW installation directory
   * @param {string} baseCommandsDir - Optional base commands directory (defaults to .claude/commands/flow)
   */
  async generateTaskToolCommands(projectDir, flowDir, baseCommandsDir = null) {
    const tasks = await this.loadTaskManifest(flowDir);
    const tools = await this.loadToolManifest(flowDir);

    // Base commands directory - use provided or default to Claude Code structure
    const commandsDir = baseCommandsDir || path.join(projectDir, '.claude', 'commands', 'flow');
  ...
  generateCommandContent(item, type) {
    ...
    if (!itemPath || typeof itemPath !== 'string') {
      ...
      itemPath = `{project-root}/${this.flowFolderName}/${item.module}/${typePlural}/${item.name}.md`;
    } else {
      // Normalize path separators to forward slashes
      itemPath = itemPath.replaceAll('\\', '/');

      // Extract relative path from absolute paths (Windows or Unix)
      // Look for _flow/ or flow/ in the path and extract everything after it
      // Match patterns like: /_flow/core/tasks/... or /flow/core/tasks/...
      // Use [/\\] to handle both Unix forward slashes and Windows backslashes,
      // and also paths without a leading separator (e.g., C:/_flow/...)
      const flowMatch = itemPath.match(/[/\\]_flow[/\\](.+)$/) || itemPath.match(/[/\\]flow[/\\](.+)$/);
      if (flowMatch) {
        // Found /_flow/ or /flow/ - use relative path after it
        itemPath = `{project-root}/${this.flowFolderName}/${flowMatch[1]}`;
      } else if (itemPath.startsWith(`${FLOW_FOLDER_NAME}/`)) {
        // Relative path starting with _flow/
        itemPath = `{project-root}/${this.flowFolderName}/${itemPath.slice(FLOW_FOLDER_NAME.length + 1)}`;
      } else if (itemPath.startsWith('flow/')) {
        // Relative path starting with flow/
        itemPath = `{project-root}/${this.flowFolderName}/${itemPath.slice(5)}`;
      } else if (!itemPath.startsWith('{project-root}')) {
        // For other relative paths, prefix with project root and flow folder
        itemPath = `{project-root}/${this.flowFolderName}/${itemPath}`;
      }
    }
...
`;
   ...
   */
  async loadTaskManifest(flowDir) {
    const manifestPath = path.join(flowDir, '_config', 'task-manifest.csv');
   ...
   */
  async loadToolManifest(flowDir) {
    const manifestPath = path.join(flowDir, '_config', 'tool-manifest.csv');
   ...
   * Creates flat files like: flow_bmm_help.md
   *
   * @param {string} projectDir - Project directory
   * @param {string} flowDir - FLOW installation directory
   ...
   */
  async generateColonTaskToolCommands(projectDir, flowDir, baseCommandsDir) {
    const tasks = await this.loadTaskManifest(flowDir);
    const tools = await this.loadToolManifest(flowDir);
    ...
    for (const task of tasks || []) {
      const commandContent = this.generateCommandContent(task, 'task');
      // Use underscore format: flow_bmm_name.md
   ...
   * @returns {Object} Generation results
   */
  async generateDashTaskToolCommands(projectDir, flowDir, baseCommandsDir) {
    const tasks = await this.loadTaskManifest(flowDir);
    const tools = await this.loadToolManifest(flowDir);
    ...
    for (const task of tasks || []) {
      const commandContent = this.generateCommandContent(task, 'task');
      // Use dash format: flow-bmm-name.md
  ...
  async writeColonArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'task' || artifact.type === 'tool') {
        ...
        // Use underscore format: flow_module_name.md
   ...
   * Creates flat files like: flow-bmm-help.md
   *
   * Note: Tasks/tools do NOT have flow-agent- prefix - only agents do.
   *
   * @param {string} baseCommandsDir - Base commands directory for the IDE
   * @param {Array} artifacts - Task/tool artifacts with relativePath
   * @returns {number} Count of commands written
  ...
  async writeDashArtifacts(baseCommandsDir, artifacts) {
    ...
    for (const artifact of artifacts) {
      if (artifact.type === 'task' || artifact.type === 'tool') {
        const commandContent = this.generateCommandContent(artifact, artifact.type);
        // Use dash format: flow-module-name.md
