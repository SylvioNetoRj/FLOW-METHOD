...
class IdeManager {
  ...
  /**
   * Set the flow folder name for all IDE handlers
  ...
  setFlowFolderName(flowFolderName) {
    ...
    for (const handler of this.handlers.values()) {
      if (typeof handler.setFlowFolderName === 'function') {
        handler.setFlowFolderName(flowFolderName);
      }
    }
  }
  ...
  async loadConfigDrivenHandlers() {
    ...
    for (const [platformCode, platformInfo] of Object.entries(platformConfig.platforms)) {
      ...
      if (typeof handler.setFlowFolderName === 'function') {
        handler.setFlowFolderName(this.flowFolderName);
      }
   ...
   */
  async setup(ideName, projectDir, flowDir, options = {}) {
    ...
    try {
      const handlerResult = await handler.setup(projectDir, flowDir, options);
