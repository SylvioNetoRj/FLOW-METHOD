/**
 * FLOW Documentation Build Pipeline
 *
...
const REPO_URL = 'https://github.com/SylvioNetoRj/FLOW-METHOD';
...
// Main Entry Point
/**
 * Orchestrates the full FLOW documentation build pipeline.
 *
...
async function main() {
  ...
  console.log();
  printBanner('FLOW Documentation Build Pipeline');
  console.log();
...
function generateLlmsTxt(outputDir) {
  ...
  const content = [
    ...
    '## Quick Start',
    '',
    `- **[Getting Started](${siteUrl}/tutorials/getting-started/)** - Tutorial: install and learn how FLOW works`,
    `- **[Installation](${siteUrl}/how-to/install-flow/)** - How to install FLOW Method`,
    '',
    '## Core Concepts',
    '',
...
function generateLlmsFullTxt(docsDir, outputDir) {
  ...
  const output = [
    '# FLOW Method Documentation (Full)',
    '',
